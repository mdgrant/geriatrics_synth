...
| 2023-11-09 | Added balance tables (also appear in appendix).|
...
| 2024-01-05 | Limited results to neuraxial versus general anesthesia (37 RCTs, 38 publications). |
...
# exclude trials with blocks Weber 2009, Kasprzak 2006, Lewis 2008, Sellbrant 2021, Sellbrant 2021, Gu 2021, Aasbo 2002, Hestin 2022
...
dichot_freq_fun(dichot_dat, "ADL: activities of daily living; DNCR: delayed neurocognitive recovery; PND: postoperative neurocognitive disorder; QoR: quality of recovery; ") |>
  tab_footnote("Number or trial publications, not the unique trials (see inlcuded studies below).", locations = cells_column_labels(columns = stat_1))
```
...
included_dat <- study_char_dat |>
  ...
  mutate(
    ...
    # edit levels by kq
    surgs = factor(surgs, levels = c("Ortho", "Urol", "GI/Abdominal", "Neuro", "Gyn", "Gyn|Ortho|Urol|Vasc", "Ophtho", "Other", "Spine", "Various", "Vasc"))
  ) |>
...
included_dat |>
  ...
  tab_footnote("GI: gastrointestinal; Ortho: orthopedic; Urol: urological; Neuro: neurological; Oralmax: oral maxillofacial; Vasc: vascular; Hip fracture: only hip fracture.")
...
reg_gen_tab |>
  ...
  group_by(surgs) |>
  gt(id = "one") |>
  row_group_order(groups = c("Ortho — HipFx", "Ortho — HipFx|Other", "Ortho — TKA|THA|HipFx|Other", "Ortho — TKA", "Ortho — THA", "Ortho", "Urol", "GI/Abdominal", "Neuro", "Gyn", "Gyn|Ortho|Urol|Vasc", "Other", "Various TKA|THA|Other", "Vasc")) |>
  # "GI/Abdominal", "Gyn", "Gyn|Ortho|Urol|Vasc", "Neuro", "Ortho", "Ortho — HipFx", "Ortho — HipFx|Other", "Ortho —  THA", "Ortho — TKA", "Ortho — TKA|THA|HipFx|Other", "Other", "Urol", "Various TKA|THA|Other", and "Vasc".
  cols_hide(c(refid, arm_id, study, year, ortho, reg_ns, reg_block)) |>
  cols_label(
    study_l          = "Study",
    arm_n            = " N",
    arm              = "Comparator",
...
<caption_mg> `r figure_ref()` Neuraxial versus general anesthesia. </caption_mg>

```{r delirium_meta_forest}
#| echo: false
#| include: false
png("assets/kq3_delirium_meta_forest.png", width = 9.7, height = 7.1, units = "in", res = 300)
...
![](assets/kq3_delirium_meta_forest.png){#fig-delirium-main fig.align="left" width="72%"}

<foot_mg>RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- . <br/> Risk ratios not applying the Hartung-Knapp adjustment --- hip fracture 1.05 (95% CI, 0.86--1.27), others 0.74 (95% CI: 0.35--1.60); random effects pooled estimate weights — Neuman 2021 (25.2%), Brown 2021 (17.3%), Li 2022 (16.8%), and Shin 2020 (11.7%).<br/> `r paste0("Harbord test for small study effects P = ", digit2(temp_metabias$pval), ".")`<br/> `r paste0("Delirium was designated a primary or secondary outcome in ", prim_sec_out_fun(prim_delirium, delirium_meta_dat$refid), " and ", prim_sec_out_fun(sec_delirium, delirium_meta_dat$refid), ", respectively.")`</foot_mg>  

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
####    <caption_mg> `r figure_ref()` Neuraxial versus general anesthesia (risk difference). </caption_mg> <a id="risk-diff"></a>

```{r delirium_meta_forest_rd}
#| echo: false
#| include: false
png("assets/kq3_delirium_meta_forest_rd.png", width = 9.7, height = 7.1, units = "in", res = 300)
...
png("assets/kq3_delirium_cummeta.png", width = 8.05, height = 3.2, units = "in", res = 300, pointsize = 10)
...
![](assets/kq3_delirium_cummeta.png){fig.align="left" width="61%"}
:::
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` General anesthesis event rate (control arm/baseline risk) and risk ratios. </caption_mg> <a id="bubble-plot"></a>
...
![](assets/gen_reg_delirium_meta_bubble.png){width="75%"}

<foot_mg> No suggestion for RR dependence on baseline risk. Asterisks indicate trials limited to hip fracture repair.</foot_mg> 

:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Summary risk of bias from randomized clinical trials comparing neuraxial to general anesthesia (weighted). </caption_mg>
...
<caption_mg> `r table_ref()` Delayed neurocognitive recovery incidence and ascertainment in randomized clinical trials comparing neuraxial to general anesthesia. </caption_mg>
...
<caption_mg> `r figure_ref()` Postoperative neurocognitive disorder (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>
...
png("assets/kq3_gen_reg_dncr.png", width = 9.65, height = 3.4, units = "in", res = 300)
...
![](assets/kq3_gen_reg_dncr.png){width="71%"}
...
<caption_mg> `r table_ref()` Postoperative neurocognitive disorder incidence and ascertainment in a randomized clinical trial comparing neuraxial to general anesthesia.</caption_mg>
...
ncd_dat |>
  ...
  tab_style(style = cell_text(align = "left"),        locations = cells_column_labels(columns = c(study, arm_reg))) |>
  tab_style(style = cell_text(align = "center"),      locations = cells_column_labels(columns = c(starts_with("scale"), pre_mmse, ncd_time, rr_ci))) |>
  tab_style(style = cell_text(align = "right"),       locations = cells_column_labels(columns = c(neurocog_nper))) |>
  tab_style(style = cell_text(align = "left"),        locations = cells_body(columns = c(study, arm_reg, pre_mmse))) |>
...
<caption_mg> `r table_ref()` Physical functional status in a randomized clinical trial comparing neuraxial to general anesthesia. </caption_mg>
...
<caption_mg> `r table_ref()` Reported complications for neuraxial versus general anesthesia --- cardiovascular, stroke, renal, pulmonary, and reintubation. </caption_mg>
...
<caption_mg> `r figure_ref()` **Risk ratio** for myocardial infarction (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_mi}
#| echo: false
#| include: false

png("assets/kq3_complication_mi.png", width = 7.9, height = 3.6, units = "in", res = 300)
temp_meta <- complication_meta_fun("MI")
complication_forest_fun(temp_meta, "Similar arms combined for Liang 2017.")
dev.off()
```
...
<caption_mg> `r figure_ref()` **Risk difference** for myocardial infarction (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_mi_rd}
#| echo: false
#| include: false

png("assets/kq3_complication_mi_rd.png", width = 7.8, height = 3.6, units = "in", res = 300)
temp_meta <- complication_meta_riskdiff_fun("MI")
complication_forest_riskdiff_fun(temp_meta, "Similar arms combined for Liang 2017.")
dev.off()
```

![](assets/kq3_complication_mi_rd.png){fig.align="left" width="58%"}
<br/>

<!-- #### Cardiac Arrest -->

<!-- <caption_mg> `r figure_ref()` **Risk difference** for cardiac arrest (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg> -->

```{r complication_cardarr_rd}
#| eval: false
#| echo: false
#| include: false

# png("assets/kq3_complication_cardarr_rd.png", width = 7.7, height = 2.5, units = "in", res = 300)
...
<!-- ![](assets/kq3_complication_cardarr_rd.png){fig.align="left" width="58%"} -->

#### Stroke

<caption_mg> `r figure_ref()` **Risk ratio** for stroke (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_stroke}
#| echo: false
#| include: false

png("assets/kq3_complication_stroke.png", width = 7.9, height = 2.8, units = "in", res = 300)
temp_meta <- complication_meta_fun("Stroke", use_hakn = FALSE, predictive = FALSE)
complication_forest_fun(temp_meta, predictive = FALSE)
dev.off()
...
<caption_mg> `r figure_ref()` **Risk difference** for stroke (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_stroke_rd}
#| echo: false
#| include: false

png("assets/complication_stroke_rd.png", width = 7.98, height = 3.0, units = "in", res = 300)
temp_meta <- complication_meta_riskdiff_fun("Stroke", use_hakn = FALSE, predictive = TRUE)
complication_forest_riskdiff_fun(temp_meta, predictive = TRUE)
dev.off()

```
![](assets/complication_stroke_rd.png){fig.align="left" width="58%"}

#### Renal

<caption_mg> `r figure_ref()` **Risk ratio** for renal complications (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_renal}
#| echo: false
#| include: false
 
png("assets/complication_renal.png", width = 7.88, height = 3.2, units = "in", res = 300)
temp_meta <- complication_meta_fun("Renal", use_hakn = FALSE, predictive = FALSE)
complication_forest_fun(temp_meta)
dev.off()

```
![](assets/complication_renal.png){fig.align="left" width="58%"}

<caption_mg> `r figure_ref()` **Risk difference** for renal complications (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_renal_rd}
#| echo: false
#| include: false

png("assets/complication_renal_rd.png", width = 7.9, height = 3.2, units = "in", res = 300)
temp_meta <- complication_meta_riskdiff_fun("Renal", use_hakn = FALSE, predictive = FALSE)
complication_forest_riskdiff_fun(temp_meta)
dev.off()

```
![](assets/complication_renal_rd.png){fig.align="left" width="58%" }

<!-- #### Pulmonary -->

<!-- <caption_mg> `r figure_ref()` **Risk ratio** for pulmonary complications (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg> -->

```{r complication_pulmonary}
#| eval: false
#| echo: false
#| include: false

png("assets/complication_pulmonary.png", width = 7.76, height = 2.4, units = "in", res = 300)
temp_meta <- complication_meta_fun("Pulmonary", use_hakn = FALSE, predictive = FALSE)
complication_forest_fun(temp_meta, random = FALSE)
dev.off()

```
<!-- ![](assets/complication_pulmonary.png){fig.align="left" width="57%" } -->

<!-- <caption_mg> `r figure_ref()` **Risk difference** for pulmonary complications (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg> -->

```{r complication_pulmonary_rd}
#| eval: false
#| echo: false
#| include: false

png("assets/complication_pulmonary_rd.png", width = 7.97, height = 2.4, units = "in", res = 300)
temp_meta <- complication_meta_riskdiff_fun("Pulmonary", use_hakn = TRUE, predictive = FALSE)
complication_forest_riskdiff_fun(temp_meta, random = FALSE)
dev.off()

```
<!-- ![](assets/complication_pulmonary_rd.png){fig.align="left" width="59%"} -->

#### Pneumonia

<caption_mg> `r figure_ref()` **Risk ratio** for pneumonia (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_pneumonia}
#| echo: false
#| include: false

png("assets/complication_pneumonia.png", width = 7.9, height = 4.2, units = "in", res = 300)
temp_meta <- complication_meta_fun("Pneumonia", use_hakn = TRUE, predictive = FALSE)
# temp_meta <- update(temp_meta, subgroup = surg_hip_oth)
complication_forest_fun(temp_meta, "Similar arms combined for Liang 2017. Hip fracture studies only RR 0.48 (0.33–0.70).", text.addline2 = "Continuity correction added when 0 events in one arm.")
dev.off()

```
![](assets/complication_pneumonia.png){fig.align="left" width="58%"}

<caption_mg> `r figure_ref()` **Risk difference** for pneumonia (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_pneumonia_rd}
#| echo: false
#| include: false

png("assets/complication_pneumonia_rd.png", width = 8.1, height = 4.0, units = "in", res = 300)
...
```
![](assets/complication_pneumonia_rd.png){fig.align="left" width="59%"}
...
<caption_mg> `r figure_ref()` **Risk ratio** for pulmonary embolism (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_pe}
#| echo: false
#| include: false

png("assets/complication_pe.png", width = 7.9, height = 3.9, units = "in", res = 300)
temp_meta <- complication_meta_fun("PE", use_hakn = TRUE, predictive = FALSE)
complication_forest_fun(temp_meta, "Similar arms combined for Liang 2017.")
dev.off()

```
![](assets/complication_pe.png){fig.align="left" width="58%"}

<caption_mg> `r figure_ref()` **Risk difference** for pulmonary embolism (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>

```{r complication_pe_rd}
#| echo: false
#| include: false

png("assets/complication_pe_rd.png", width = 7.9, height = 3.9, units = "in", res = 300)
temp_meta <- complication_meta_riskdiff_fun("PE", use_hakn = TRUE, predictive = FALSE)
complication_forest_riskdiff_fun(temp_meta, "Similar arms combined for Liang 2017.")
dev.off()

```
![](assets/complication_pe_rd.png){fig.align="left" width="58%"}

```{r complication_pulmonary-congestion}
#| eval: false
# only Neuman
...
<caption_mg> `r figure_ref()` Risk ratio comparing higher versus lower categories of patient satisfaction (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>
...
# summary(plac_meta)
png("assets/satis_meta.png", width = 8.12, height = 4.4, units = "in", res = 300)
forest(satis_meta,
  allstudies = TRUE,
  # random = FALSE,
  label.e = "Neuraxial     ",
...
![](assets/satis_meta.png){fig.align="left" width="60%"}
...
<caption_mg> `r figure_ref()` Mean difference in lengths of stay (neuraxial versus general anesthesia; randomized clinical trials). </caption_mg>
...
png("assets/kq3_los_meta.png", width = 10.2, height = 5.2, units = "in", res = 300)
forest(los_meta,
  sortvar = country,
  weight.study = "random",
  common =  FALSE,
...
<caption_mg> `r figure_ref()` **Risk ratio** for hospital or 30-day mortality (neuraxial versus general; randomized clinical trials). </caption_mg>
...
# summary(plac_meta)
png("assets/mort_rct_meta.png", width = 8.8, height = 7.1, units = "in", res = 300)
forest(mort_rct_meta,
  ...
  xlab = "Favors: Neuraxial                      General     ",
  addrows.below.overall = 4,
  text.addline1 = "Hartung-Knapp adjustment not applied. Overall random effects estimate with adjustment RR 0.64 (95% CI, 0.48–1.48)."
)
dev.off()

```
![](assets/mort_rct_meta.png){fig.align="left" width="65%"}

<br/>

<caption_mg> `r figure_ref()` **Risk difference** for hospital or 30-day mortality (neuraxial versus general; randomized clinical trials). </caption_mg>

```{r mort_rct_rd_meta}
#| echo: false
#| include: false 

png("assets/mort_rct_rd_meta.png", width = 8.84, height = 6.9, units = "in", res = 300)
...
```
![](assets/mort_rct_rd_meta.png){fig.align="left" width="65%"}
...
<caption_mg> `r figure_ref()` **Risk ratio** for 90-day mortality (neuraxial versus general; randomized clinical trials). </caption_mg>
...
png("assets/mort_rct_90d_meta.png", width = 8.84, height = 3.05, units = "in", res = 300)
forest(mort_rct_meta,
  allstudies = TRUE,
  # random = FALSE,
  label.e = "Neuraxial",
...
```
![](assets/mort_rct_90d_meta.png){fig.align="left" width="65%"}

<caption_mg> `r figure_ref()` **Risk difference** for 90-day mortality (neuraxial versus general; randomized clinical trials). </caption_mg>

```{r mort_rct_rd_90dmeta}
#| echo: false
#| include: false

png("assets/mort_rct_rd_90dmeta.png", width = 8.74, height = 3, units = "in", res = 300)
...
```
![](assets/mort_rct_rd_90dmeta.png){fig.align="left" width="64%"}
