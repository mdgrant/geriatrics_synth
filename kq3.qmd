...
pairwise_dat <- pairwise(
  ...
  sm = "RR") |> 
  select(studlab:country, surg_hip_oth) 

del_meta_dat <- pairwise_dat |>
  left_join(rob2_meta_dat, by = "refid") |>
  select(study, year, event2, n2, event1, n1, D1:Overall, surg_hip_oth) |>
  mutate(
    sample_size = n1 + n2,
    rob = case_when(
      Overall == "+" ~ "Low",
...
```{r delirium_meta_forest, fig.width = 14, fig.height = 7.0, fig.align = "left", warning = TRUE}
...
In hip fracture subgroup not applying the Hartung-Knapp adjustment — RR 1.03 (95% CI, 0.85–1.24); random effects pooled estimate dominated by Neuman 2021 (74.1%) and Li 2022 (13.0%).<br/>

`r paste0("Harbord test for small study effects P = ", digit2(temp_metabias$pval), ".")`<br/> `r paste0("Delirium a designated primary outcome in ", prim_sec_out_fun(prim_delirium, delirium_meta_dat$refid), "; a secondary outcome in ", prim_sec_out_fun(sec_delirium, delirium_meta_dat$refid), ".")`</foot_mg>
...
<!-- ::: {.callout-note collapse="true" appearance="minimal" icon="false"} -->
<!-- #### <font size = 4> General versus regional anesthesia pooled with subgroups (randomized clinical trials). </font> -->
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Summary risk of bias from randomized clinical trials comparing regional and general anesthesia. </font>
...
<font size = 4> `r table_ref()` Cognitive dysfunction ≤ 30 days incidence and ascertainment in trials comparing regions and general anesthesia. </font>
...
<font size = 4> `r figure_ref()` Cognitive dysfunction > 30 days  (regional versus general anesthesia; randomized clinical trials). </font>
...
<font size = 4> `r table_ref()` Cognitive dysfunction > 30 days and ascertainment. </font>
...
adverse_events_gt_dat <- adverse_events_dat |>
  ...
  mutate(complication = factor(complication, levels = c("cardiac", "myoinfarct", "cardarr", "stroke", "kidneyinj", "pulm", "pneum", "pulcongest", "pe", "intub"), labels = c("Other Cardiac", "MI", "Cardiac Arrest", "Stroke", "AKI", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "PE", "Reintubation"))) |>
  arrange(year, study, arm) 
...
adverse_events_gt_dat |> 
  ...
  gt(id = "one") |>
  row_group_order(groups = c("MI", "Cardiac Arrest", "Other Cardiac", "Stroke", "AKI", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "PE", "Reintubation")) |>
...
complication_meta <- c("Cardiac", "MI", "Cardiac Arrest", "Stroke", "AKI", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "PE", "Reintubation")
...
complication_forest_fun <- function(adverse_meta){
forest(adverse_meta,
  allstudies = TRUE,
  random = TRUE,
  label.e = "Regional ",
...
<!-- <font size = 4> `r figure_ref()` Risk ratio for composite measures of cardiac complications (regional versus general anesthesia; randomized clinical trials). </font> -->

<!-- ```{r complication_cardiac,  fig.width = 14, fig.height = 4.5, fig.align = "left"} -->
<!-- temp_meta <- complication_meta_fun("Cardiac") -->
<!-- complication_forest_fun(temp_meta) -->
<!-- ``` -->
<!-- <foot_mg> RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- .<br/> -->
<!-- Similar arms combined for Shin 2020 and Bielka 2021. -->
<!-- </foot_mg>  -->

#### Myocardial Infarction

<font size = 4> `r figure_ref()` Risk ratio for myocardial infarction (regional versus general anesthesia; randomized clinical trials). </font>

```{r complication_mi,  fig.width = 14, fig.height = 3.5, fig.align = "left"}
...
<font size = 4> `r figure_ref()` Risk ratio for stroke (regional versus general anesthesia; randomized clinical trials). </font>

```{r complication_stroke,  fig.width = 14, fig.height = 3.5, fig.align = "left"}
...
<font size = 4> `r figure_ref()` Risk ratio for acute kidney injury (regional versus general anesthesia; randomized clinical trials). </font>

```{r complication_aki,  fig.width = 14, fig.height = 2.75, fig.align = "left"}
temp_meta <- complication_meta_fun("AKI", use_hakn = FALSE, predictive = FALSE)
complication_forest_fun(temp_meta)
```
<foot_mg> RR: risk ratio; ; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- .<br/>
</foot_mg> 
...
<font size = 4> `r figure_ref()` Risk ratio for pulmonary complications (regional versus general anesthesia; randomized clinical trials). </font>
