...
<font size = 4> `r figure_ref()` Risk differences in complications — cardiac, stroke, pulmonary, and acute kidney injury (regional versus general; randomized clinical trials). </font>
...
satis_dat_gt |> 
 mutate(study_l = ifelse(row_number() > 1, "", study_l), 
  ...
  tab_style(style = cell_text(align = "center"),      locations = cells_column_labels(columns = c(asa_ps_incl, age_table))) |> 
  tab_style(style = cell_text(align = "right"),       locations = cells_column_labels(columns = c(n_percent))) |> 
  tab_style(style = cell_text(align = "left"),        locations = cells_body(columns = c(arm))) |>
  tab_style(style = cell_text(align = "center"),        locations = cells_body(columns = c(rd_ci))) |>
...
temp <- rob_traffic_light(rob_temp_dat, psize = 4, tool = "ROB2", colour = "colourblind")
```
