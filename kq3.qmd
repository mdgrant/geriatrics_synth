...
## Recent updates (last 2024-01-02)
...
| 2024-01-02 | Added [figure 4](kq3.html#figure-4) to display potential dependence of effects on baseline (control arm event rate) for delirium. Updated balance tables|
:::
...
kq3_refid <- kq3_refid[!kq3_refid %in% c(813, 3434, 16596)] # retrospective cohort and Neuman 2016
...
study_arm_dat <- data_kq(study_arm_dat, kq3_refid) |>
  ...
  mutate(
    ...
    kq3_anesth_reg = ifelse(!is.na(kq3_reg_block), "Block", NA),
...
<font size = 4> Benefits, harms, and strength of evidence (GRADE) for regional versus general anesthesia. </font>
...
<font size = 4> Included complications and strength of evidence (GRADE) for regional versus general anesthesia. </font>
...
<caption_mg> `r table_ref()` Selected characteristics of randomized clinical trials comparing regional to general anesthesia. </caption_mg>
...
reg_gen_tab |>
  ...
  cols_hide(c(refid, arm_id, study, year, ortho, reg_ns)) |>
  cols_label(
    study_l          = "Study",
    arm_n            = " N",
    arm              = "Comparator",
...
<caption_mg> `r table_ref()` Delirium incidence and ascertainment during hospitalization in randomized clinical trials comparing regional to general anesthesia. </caption_mg>
...
<caption_mg> `r figure_ref()` Regional versus general anesthesia (randomized clinical trials). </caption_mg>

```{r delirium_meta_forest}
#| echo: false
#| include: false
png("assets/kq3_delirium_meta_forest.png", width = 9.7, height = 7.5, units = "in", res = 300)
...
#### <caption_mg> `r figure_ref()` Regional versus general anesthesia cumulative meta-analysis by study size. </caption_mg>

```{r delirium_meta_forest_cum}
#| warning: true
#| echo: false
#| include: false
png("assets/kq3_delirium_cummeta.png", width = 8.38, height = 3.6, units = "in", res = 300, pointsize = 10)
...
![](assets/kq3_delirium_cummeta.png){width="61%"}
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Regional versus general anesthesia small study effects. </caption_mg>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` General anesthesis event rate (control arm/baseline risk) and risk ratios. </caption_mg> <a id="figure-4"></a>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Summary risk of bias from randomized clinical trials comparing regional to general anesthesia (weighted). </caption_mg>
...
<caption_mg> `r table_ref()` Delayed neurocognitive recovery incidence and ascertainment in randomized clinical trials comparing regional to general anesthesia. </caption_mg>
...
<caption_mg> `r figure_ref()` Postoperative neurocognitive disorder (regional versus general anesthesia; randomized clinical trials). </caption_mg>
...
<caption_mg> `r table_ref()` Postoperative neurocognitive disorder incidence and ascertainment in a randomized clinical trial comparing regional to general anesthesia.</caption_mg>
...
<caption_mg> `r table_ref()` Physical functional status in a randomized clinical trial comparing regional to general anesthesia. </caption_mg>
...
<caption_mg> `r table_ref()` Reported complications for regional versus general anesthesia --- cardiovascular, stroke, renal, pulmonary, and reintubation. </caption_mg>
...
complication_forest_fun <- function(adverse_meta, addline = "", random = TRUE){
forest(adverse_meta,
  allstudies = TRUE,
  random = random,
  label.e = "Regional ",
...
complication_forest_riskdiff_fun <- function(adverse_meta, addline = "", random = TRUE){
forest(adverse_meta,
  allstudies = TRUE,
  random = random,
  label.e = "Regional ",
...
png("assets/kq3_complication_mi.png", width = 7.9, height = 3.8, units = "in", res = 300)
...
png("assets/kq3_complication_mi_rd.png", width = 7.8, height = 3.8, units = "in", res = 300)
...
png("assets/kq3_complication_stroke.png", width = 7.9, height = 3.8, units = "in", res = 300)
temp_meta <- complication_meta_fun("Stroke", use_hakn = FALSE)
complication_forest_fun(temp_meta, "85% of events occurred in the GALA carotid surgery trial (Lewis 2008).")
dev.off()
...
png("assets/complication_stroke_rd.png", width = 7.98, height = 3.8, units = "in", res = 300)
temp_meta <- complication_meta_riskdiff_fun("Stroke", use_hakn = FALSE)
...
png("assets/complication_pneumonia.png", width = 7.9, height = 6.4, units = "in", res = 300)
temp_meta <- complication_meta_fun("Pneumonia", use_hakn = TRUE, predictive = FALSE)
temp_meta <- update(temp_meta, subgroup = surg_hip_oth)
complication_forest_fun(temp_meta, "Similar arms combined for Bielka 2021 and Liang 2017." )
dev.off()
...
png("assets/complication_pneumonia_rd.png", width = 8.1, height = 6.4, units = "in", res = 300)
temp_meta <- complication_meta_riskdiff_fun("Pneumonia", use_hakn = TRUE, predictive = FALSE)
temp_meta <- update(temp_meta, subgroup = surg_hip_oth)
complication_forest_riskdiff_fun(temp_meta, "Similar arms combined for Bielka 2021 and Liang 2017." )
dev.off()
...
png("assets/complication_pe.png", width = 7.9, height = 4.2, units = "in", res = 300)
...
png("assets/complication_pe_rd.png", width = 7.9, height = 4.2, units = "in", res = 300)
...
<caption_mg> `r table_ref()` Patient satisfaction comparing higher versus lower categories according to regional or general anesthesia. </caption_mg>
...
<caption_mg> `r figure_ref()` Risk ratio comparing higher versus lower categories of patient satisfaction (Regional versus general anesthesia; randomized clinical trials). </caption_mg>
...
png("assets/kq3_los_meta.png", width = 10.2, height = 5.7, units = "in", res = 300)
forest(los_meta,
  weight.study = "random",
  common =  FALSE,
  label.e = "Reg     ",
  ...
  addrows.below.overall = 4,
  text.addline1 = "When the mean or standard deviation were not reported, they were imputed from from the median, interquartile range, range, or between-arm p-value."
)
dev.off()
```
...
<caption_mg> `r table_ref()` Discharge location in studies comparing regional with general anesthesia. </caption_mg>
...
# summary(plac_meta)
png("assets/mort_rct_meta.png", width = 8.8, height = 7.5, units = "in", res = 300)
forest(mort_rct_meta,
  ...
  xlab = "Favors: Regional                      General     ",
  addrows.below.overall = 4,
  text.addline1 = "Hartung-Knapp adjustment not applied. Overall random effects estimate with adjustment RR 0.68 (95% CI, 0.43–1.07)."
)
dev.off()
...
png("assets/mort_rct_rd_meta.png", width = 8.84, height = 7.3, units = "in", res = 300)
