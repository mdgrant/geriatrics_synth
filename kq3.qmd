...
reg_gen_tab <- study_arm_dat |>
  ...
  mutate(
    ...
    volatile = ifelse(kq3_gen_type == "inhalation", "\U2715", NA),
    tiva = ifelse(kq3_gen_type == "tiva", "\U25CF", NA),
    tiva_inhal = ifelse(kq3_gen_type == "tiva_inhal", "\U2713", NA),
    tiva_inhal = ifelse(kq3_gen_type == "tiva_inhal", "\U2713", NA),
    kq3_reg_block = notna_to_x(kq3_reg_block, "\U00D7"),
    notrep = ifelse(kq3_gen_type == "notrep", "\U2716", NA),
    across(c(kq3_reg_spinal, kq3_reg_epidural, kq3_reg_cse), ~ ifelse(!is.na(.x), "\U2713", NA)),
  ...
  select(refid, arm_id, study, study_l, arm_n, arm, asa_ps_incl, age_table, pre_mmse, volatile, tiva, tiva_inhal, notrep, reg_spinal, reg_epidural, reg_cse, reg_block, surgs, year, ortho) |>
...
reg_gen_tab |>
  ...
  cols_hide(c(refid, arm_id, study, year, ortho, tiva_inhal)) |>
  cols_label(
    study_l          = "Study",
    arm_n            = " N",
    arm              = "Comparator",
  ...
  tab_style(style = list(cell_text(color = color_2)), locations = cells_body(columns = c(arm_n:reg_cse), rows = str_detect(arm, "Neur"))) |>
  tab_style(style = list(cell_text(color = color_3)), locations = cells_body(columns = c(arm_n:reg_cse), rows = str_detect(arm, "Block"))) |>
  ...
  tab_footnote("Fopivacaine.", locations = cells_body(columns = arm, rows = study == "Sellbrant 2021" & arm_id == 2)) |> 
  tab_footnote("Mepivacaine.", locations = cells_body(columns = arm, rows = study == "Sellbrant 2021" & arm_id == 3)) |> 
  gtsave(filename = "/Users/mgrant/Desktop/comparators.html", inline_css = TRUE)
