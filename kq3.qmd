...
| 2024-03-16 | Dementia subgroup delirium results added. |
|            | Fixed coding error when collapsing arms for Shin 2020, that resulted in an incorrect denominator for the general anesthesia arm. |
...
|            | Added delirium results for dementia subgroups to [Figure 1](kq3.html#pooled) footnote. |
...
tiva_inhaled_dat <- readxl::read_excel("data/balance_tables_2023-09-14_mac_mg.xlsx", sheet = "RegionalGeneral", range = "A4:P21") |> # [, c(2:4, 10, 12, 13, 15:18)] |>
...
There were no changes to the adverse event results following the search update.

```{r}
#| tbl-cap: No changes to adverse events following search update.
#| eval: false
#| html-table-processing: none
...
# gen_reg_delirium_meta_nodementia <- update(gen_reg_delirium_meta,   method.random.ci = "HK", subset = refid_c %in% no_cogimpair_refid_c)
...
gen_reg_delirium_baseline_meta <- metaprop(event1, n1, data = del_meta_dat, studlab = study, method = "Inverse", prediction = TRUE, allstudies = TRUE, subset = !str_detect(study, "dementia"), subgroup = surg_hip_oth)
...
#| echo: false
#| include: false
png("assets/kq3_delirium_meta_forest.png", width = 9.95, height = 7.05, units = "in", res = 300)
...
<foot_mg>RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- . <br/> Risk ratios not applying the Hartung-Knapp adjustment --- hip fracture 1.08 (95% CI, 0.89--1.31), others 0.74 (95% CI: 0.35--1.57); random effects pooled estimate weights — Neuman 2021 (63.6%), Brown 2021 (11.9%), Li 2022 (9.4%), and Shin 2020 (5.1%).<br/> `r paste0("Harbord test for small study effects P = ", digit2(temp_metabias$pval), ".")`<br/> `r paste0("Delirium was designated a primary or secondary outcome in ", prim_sec_out_fun(prim_delirium, delirium_meta_dat$refid), " and ", prim_sec_out_fun(sec_delirium, delirium_meta_dat$refid), " of trials, respectively.")`<br/>
In subgroups from O'Brien 2023 (Neuman 2021) and Li 2022 of patients with dementia RR 1.07 (95% CI, 0.83–1.38).<br/>
In 7 studies including or reporting results for patients without dementia RR 1.03 (95% CI, 0.60–1.75; PI 0.38–2.73)
</foot_mg>  
...
<foot_mg> Incidence of postoperative delirium with general anesthesia: <br/> Overall 9.4 per 100 (95% CI, 5.2 to 16.1)<br/> Hip fracture 7.9 per 100 (95% CI, 3.5 to 17.2)<br/> Other 11.1 per 100 (95% CI, 4.1 to 26.4) </foot_mg> 
...
<foot_mg> No suggestion for RR dependence on baseline risk. Asterisks indicate trials limited to hip fracture repair. <br/> The pooled incidence of postoperative delirium with general anesthesia was 9.4 per 100 (95% CI, 5.2 to 16.1) overall, following hip fracture was 7.9 per 100 (95% CI, 3.5 to 17.2), and among other surgical procedures 11.1 per 100 (95% CI, 4.1 to 26.4)</foot_mg> 
...
<caption_mg> `r figure_ref()` Delayed neurcognitive recovery for neuraxial versus general anesthesia. </caption_mg>
...
<caption_mg> `r table_ref()` Physical functional status ascertained ascertained by various instruments in randomized clinical trials comparing neuraxial to general anesthesia. </caption_mg>
...
adl_long_dat |>
  ...
  tab_footnote("Arrow indicates better functional outcome.", locations = cells_column_labels(columns = scale)) |>
...
png("assets/kq3_function_meta.png", width = 8.75, height = 2.8, units = "in", res = 300)
...
png("assets/kq3_complication_renal.png", width = 8.1, height = 3.2, units = "in", res = 300)
...
# summary(plac_meta)
png("assets/kq3_mort_rct_meta.png", width = 9, height = 7.1, units = "in", res = 300)
...
png("assets/kq3_mort_rct_rd_meta.png", width = 9.03, height = 6.9, units = "in", res = 300)
