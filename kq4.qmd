...
complication_incl <- c("cardiac", "myoinfarct", "stroke", "kidneyinj", "pulm", "pulcongest", "pe", "pinfiltrate", "respfail")
...
<font size = 4> `r table_ref()` Reported complications in regional versus general anesthesia — cardiac, MI, stroke, pulmonary, pneumonia, acute kidney injury (TIVA vs. inhaled; randomized and nonrandomized designs). </font>
...
adverse_events_dat |>
  filter(complication %in% complication_incl) |>
  mutate(complication = factor(complication, levels = c("cardiac", "myoinfarct", "stroke", "kidneyinj", "pulm", "pulcongest", "pe", "pinfiltrate", "respfail"), labels = c("Cardiac", "MI", "Stroke", "AKI", "Pulmonary", "Pulmonary Congestion", "PE", "Pulmonary Infiltrate", "Respiratory Failure"))) |>
  ...
  # filter(str_detect(complication, "Rand")) |> tabyl(complication) |> arrange(desc(percent)) |> pull(complication) |> toString()
  group_by(complication) |>
  gt(id = "one") |>
  row_group_order(groups = c("Cardiac – Randomized Clinical Trial",  "MI – Randomized Clinical Trial", "MI – Retrospective Cohort", "Respiratory Failure – Randomized Clinical Trial", "Respiratory Failure – Retrospective Cohort", "PE – Randomized Clinical Trial", "PE – Retrospective Cohort", "Pulmonary Congestion – Retrospective Cohort", "AKI – Retrospective Cohort")) |>
...
<font size = 4> `r figure_ref()` Composite measure for cardiac complications (randomized clinical trials). </font>
...
```{r cardiac_meta, warning = FALSE, fig.width = 14, fig.height = 4, fig.align = "left"}
#| eval: true
...
<font size = 4> `r table_ref()` Reported in-hospital, 30-day, 90-day, and 1-year mortality from randomized clinical trials.</font>
...
<font size = 4> `r table_ref()` Reported in-hospital, 30-day, 90-day, and 1-year mortality from nonrandomized designs (all retrospective cohort studies).</font>
...
<font size = 4> `r figure_ref()` Risk ratio for hospital mortality (TIVA vs. inhaled; randomized clinical trials). </font>
...
<font size = 4> `r figure_ref()` Risk ratio for hospital mortality (TIVA vs. inhaled; nonrandomized designs). </font>
