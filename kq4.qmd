...
adverse_events_gt_dat <- adverse_events_dat |>
  ...
  mutate(complication = factor(complication, levels = c("cardiac", "myoinfarct", "kidneyinj", "pneum", "pulcongest", "pe", "respfail"), labels = c("Cardiac", "MI", "AKI", "Pneumonia", "Pulmonary Congestion", "PE", "Respiratory Failure"))) |>
...
adverse_meta_wide_dat <- adverse_meta_dat |> 
  ...
  mutate(
    ...
    complication_nodesign = factor(complication_nodesign, levels = c("Cardiac", "MI", "Pneumonia", "Pulmonary Congestion", "PE", "Respiratory Failure", "AKI"), labels = c("Cardiac", "MI", "Pneumonia", "Pulmonary Congestion", "Pulmonary Embolism", "Respiratory Failure", "Acute Kidney Injury"))
  )
...
<font size = 4> `r figure_ref()` Relative odds complications in TIVA versus general anesthesia — cardiac, pulmonary, and acute kidney injury (TIVA versus inhaled; combined randomized and nonrandomized designs). </font>
...
Reported patient satisfaction rate in general vs neuraxial or regional anesthesia RCTs by comparator.
...
los_tab |>
  ...
  tab_style(style = cell_text(align = "left"),   locations = cells_body(columns = c(study_l, age_table, los_table, bar, arm, asa_ps_incl))) |> 
