---
title: "TIVA versus Inhaled (volatile) Anesthesia"
editor: source
toc-title: "**TIVA versus Inhaled (volatile) Anesthesia**"
toc-location: "left"
toc-depth: 4
toc_float: 
  collapsed: false
...
<!-- | Date       | Modification                                                                                                                                                                    | -->
<!-- |:-----------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| -->
<!-- | 2024-03-05 | 11 studies (7 RCTs and 4 retrospective cohort) added from updated search. | -->
...
<!-- |2024-03-28  | Revised method to obtain estimates of risk differences when adjusted odds ratios from nonrandomize designs. Still approximate owing to pruning in those studies using propensity matching but consistent with approach for risk ratios.|. -->


<!-- ```{r} -->
<!-- #| tbl-cap: Overview of changes to primary outcomes following search update. -->
<!-- #| html-table-processing: none -->
<!-- tiva_inhaled_dat <- readxl::read_excel("data/balance_tables_2023-09-14_mac_mg.xlsx", sheet = "TIVAInhaled", range = "A4:R22")[, c(2:4, 10, 12, 13, 15:18)] |> -->
...
<!--   tab_footnote("Revised estimate from raw data; still approximate owing to pruning in those studies using propensity matching but more consistent with when a risk ratio was used.", locations = cells_body(columns = prior_est, rows = prior_est %in% c("-0.8 (-6.5 to 10.2)")))  -->
...
<!-- #| html-table-processing: none -->
<!-- tiva_inhaled_dat <- readxl::read_excel("data/balance_tables_2023-09-14_mac_mg.xlsx", sheet = "TIVAInhaled", range = "A25:R44")[, c(2:4, 10, 12, 13, 15:18)] |> -->
<!--   remove_empty(which = "cols") |> -->
<!--   clean_names() |> -->
<!--   mutate( -->
<!--     across(everything(), ~ replace(.x, is.na(.x), "")), -->
...
trim_image("kq4_delirium_meta_rct.png")
...
trim_image("kq4_delirium_meta_forest_rd.png")
```
...
temp_limit_meta <- limitmeta(delirium_meta_or)
...
trim_image("kq4_delirium_meta_nrsi.png")
...
<caption_mg> `r table_ref()` Neurocognitive disorder <30 days incidence and ascertainment (randomized and nonrandomized designs). Table includes only studies conducting cognitive testing at day 3 or later.</caption_mg>
...
<caption_mg> `r figure_ref()` Neurocognitive disorder <30 days assessed at postoperative day 5 or later (randomized clinical trials).  </caption_mg>
...
png("assets/kq4_dncr_meta.png", width = 10.67, height = 4.8, units = "in", res = 300) 
forest(dncr_tiva_inhaled_meta,
  ...
  addrows.below.overall = 4,
  fs.hetstat = 10,
  text.addline2 = rd_ci_from_meta_sub(dncr_tiva_inhaled_meta, pscale = 100, digits = 1)
)  
dev.off()
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Neurocognitive disorder <30 days assessed at postoperative day 5 or later (randomized clinical trials; risk difference per 100). </caption_mg> <a id="kq4_delirium_risk-diff"></a>

```{r kq4_dncr_meta_forest_rd}
#| echo: false
#| include: false
png("assets/kq4_dncr_meta_forest_rd.png", width = 9.85, height = 5, units = "in", res = 300)
...
trim_image("kq4_dncr_meta_forest_rd.png")
```
![](assets/kq4_dncr_meta_forest_rd.png){fig.align="left" width="72%"}
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Neurocognitive disorder <30 days assessed at postoperative day 5 or later (small study effects). </caption_mg>
...
# text(1.2, 0.05, labels = "Adjusted estimate", col = "darkgray", pos = 3, cex = .9)
par(bty = "n", xaxt = "s", yaxt = "s")
funnel(dncr_tiva_inhaled_meta_or,  pch = 20, contour = c(0.9, 0.95, 0.99), col.contour = limit_colors, studlab = TRUE, pos.studlab = 4, fs.smlab = 6, ff.smlab = "italic", xlim = c(0.21, 1.5), ylim = c(0.66, 0.0))
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Neurocognitive disorder <30 days summary risk of bias from randomized clinical trials (weighted). </caption_mg>
...
<caption_mg> `r table_ref()` Neurocognitive disorder ≥30 days (cognitive dysfunction after 30 days) and ascertainment. </caption_mg>
...
ncd_dat |>
  ...
  group_by(design_f_lab) |>
  gt(id = "one") |>
  row_group_order(groups = c("Randomized Clinical Trial", "Prospective Cohort", "Retrospective Cohort")) |>
...
<caption_mg> `r figure_ref()` Neurocognitive disorder ≥30 days (nonrandomized designs).  </caption_mg>
...
forest(ncd_meta,
  ...
  addrows.below.overall = 4,
  fs.hetstat = 10,
  text.addline2 = rd_ci_from_meta(ncd_meta, pscale = 100, digits = 1)
)  
dev.off()

trim_image("kq4_ncd_meta.png")
...
# functions to perform meta-analysis obtaining odds ratio using metagen or rr directly
...
complication_forest_or(temp_meta, xlim = c(0.2, 5.0), big.mark = ",", text.addline2 = rd_ci_from_meta_or_kq4(temp_meta, pscale = 1000, digits = 1)) 
dev.off()

trim_image("kq4_complication_mi.png")

# meta_rob_traffic_light_refid(temp_meta)
# complication_summary_rr_or("Myocardial Infarction")
# soe_meta_result_rr_or(temp_meta, "OR")
# RESUME: 2025-04-10 fix function soe_meta_rd_or
...
complication_forest_rr(temp_meta, random = FALSE, big.mark = ",", text.addline2 = rd_ci_from_meta(temp_meta, pscale = 1000, digits = 1))  
dev.off()

trim_image("kq4_complication_cardiac_arrest.png")
...
complication_forest_rr(temp_meta, big.mark = ",", use_predictive = FALSE, text.addline2 = rd_ci_from_meta(temp_meta, pscale = 1000, digits = 1))
dev.off()

trim_image("kq4_complication_brady.png")
...
<foot_mg> Insufficient data to estimate a valid prediction interval. </foot_mg> 
...
complication_forest_rr(temp_meta, big.mark = ",", text.addline2 = rd_ci_from_meta(temp_meta, pscale = 1000, digits = 1))
dev.off()

trim_image("kq4_complication_hypotension.png")
...
complication_forest_or(temp_meta, big.mark = ",", random = FALSE, text.addline2 = rd_ci_from_meta_or_kq4(temp_meta, pscale = 1000, digits = 1)) 
dev.off()

trim_image("kq4_complication_stroke.png")
...
complication_forest_or(temp_meta, big.mark = ",", text.addline2 = rd_ci_from_meta_or_kq4(temp_meta, pscale = 1000, digits = 1)) 
dev.off()

trim_image("kq4_complication_renal.png")
...
temp_meta <- complication_meta_rr("Pneumonia", use_hakn = TRUE, predictive = TRUE)
complication_forest_rr(temp_meta, use_predictive = TRUE, text.addline2 = rd_ci_from_meta(temp_meta, pscale = 1000, digits = 1))
dev.off()

trim_image("kq4_complication_pneumonia.png")
...
![](assets/kq4_complication_pneumonia.png){fig.align="left" width="60%"}
...
complication_forest_or(temp_meta, use_predictive = TRUE, big.mark = ",", text.addline2 = rd_ci_from_meta_or_kq4(temp_meta, pscale = 1000, digits = 1)) 
dev.off()

trim_image("kq4_complication_pe.png")
...
trim_image("kq4_respiratory_failure.png")
...
forest(mort_rct_meta,
  ...
  big.mark = ",",
  method.random.ci = "HK",
  text.addline1 = risk_diff_meta_rr_kq4()
)
dev.off()

trim_image("kq4_mort_rct.png")
...
# rd_ci_from_meta_rr_kq4(temp_meta, digits = 2)
...
forest(mort_nrsi_meta,
  ...
  big.mark = ",",
  method.random.ci = "HK",
  text.addline2 = paste0("Approximate pooled risk difference ", rd_ci_from_meta_rr_kq4(temp_meta, digits = 2))
)
dev.off()

trim_image("kq4_mort_nrsi.png")
