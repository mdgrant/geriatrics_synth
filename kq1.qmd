...
<caption_mg> Outcomes reported in additional studies.  </caption_mg>
```{r}
tribble(
...
study_char_dat |>
  ...
  tab_footnote("Studies with multiple publications counted only once (applies to 2 studies with 2 publications).")
```
...
study_char_dat |>
  ...
  tab_footnote("Studies with more that one publications considered once.",  locations = cells_body(columns = c("stat_0"), rows = label %in% c("Netherlands", "USA"))) 
...
studychar_tab <- study_arm_dat |>
  ...
  left_join(surgs |> select(refid, surgs_single_f_lump), by = "refid") |>
  rename(surgs = surgs_single_f_lump) |>
  ...
  mutate(
    ...
    #   levels = c("HipFx — Randomized Clinical Trial", "HipFx — Before-After/Time Series", "HipFx — Nonrandomized Trial", "Other Ortho — Before-After/Time Series", "GI/Abd — Randomized Clinical Trial", "GI/Abd — Before-After/Time Series", "GI/Abd — Retrospective Cohort", "Neuro — Before-After/Time Series", "Urol — Before-After/Time Series", "Various — Randomized Clinical Trial", "Various — Before-After/Time Series", "Various — Prospective Cohort")
    # ),
...
expanded_combined |> 
  ...
  # tab_footnote("Note if a comprehensive geriatric assessment was conducted, all components were presumed addressed.")
...
delirium_tab |> 
  ...
  tab_style(style = list(cell_text(color = color_2)),   locations = cells_body(columns = c(arm_n:n_percent), rows = str_detect(arm, "Exp"))) |>
  tab_footnote("RR: risk ratio; MMSE: Mini-Mental State Examination; CAM: Confusion Assessment Method; DI: Delirium Index; DRS: Delirium Rating Scale; DOS: delirium observation screening; DSM: Diagnostic and Statistical Manual of Mental Disorders; NR: not reported; HipFx: hip fracture; GI/Abd: gastrointestinal or abdominal.") |>
...
forest(delirium_rct_meta,
  ...
  text.addline1 = "Hip fracture surgery only: RR 0.84 (95% CI, 0.71-0.99; I² = 7%)",
  text.addline2 =  "Other surgery: RR 0.54 (95% CI, 0.33-0.89; I² = 0%) (without Hartung-Knapp adjustment for either)"
)
```
...
```{r delirium_meta_nrsi_forest, fig.width = 14, fig.height = 4.25, fig.align = "left", warning = TRUE}
...
```{r exp_std_rct_forest, fig.width = 14, fig.height = 6, fig.align = "left", warning = TRUE}
...
```
<foot_mg> Multilevel model fitted accounting for study reporting at more than one time point. </foot_mg> 
...
```{r exp_std_nrsi_forest, fig.width = 14, fig.height = 6.2, fig.align = "left", warning = TRUE}
...
adverse_events_gt_dat |> 
  ...
  tab_footnote("Pneumonia, infectious exacerbation of chronic obstructive pulmonary disease (COPD).", locations = cells_body(columns = c(study_l), rows = detail_pulmonary == "pneumonia, infectious exacerbation of chronic obstructive pulmonary disease (COPD)" & arm_id == 1))
...
```
<foot_mg> Hempenius 2013 and Watne 2014 unspecified; Partidge 2017 pneumonia, infectious exacerbation of chronic obstructive pulmonary disease (COPD). </foot_mg> 
...
```{r los_nrsi_meta, fig.width = 14, fig.height = 8.5, fig.align = "left"}
los_meta <- metacont(
  n.e = arm_n_Exp, 
  n.c = arm_n_Std,
  common =  TRUE,
...
```{r dc_loc_meta, fig.width = 14, fig.height = 5.5, fig.align = "left", warning = TRUE}
...
```{r dc_loc_nrsi_meta, fig.width = 14, fig.height = 6, warning = TRUE}
#| fig-align: "left"
...
```{r mort_rct_meta, fig.width = 14, fig.height = 4.8, fig.align = "center"}
...
```{r mort_nrsi_meta, fig.width = 14, fig.height = 9.5, fig.align = "center"}
...
forest(mort_nrsi_meta,
  ...
  xlab = "Favors: Expanded                         Standard        ",
  addrows.below.overall = 4,
  text.addline1 = "Hartung-Knapp adjustment not applied.",
)
```
