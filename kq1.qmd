...
<!-- | 2024-03-30 | •  Correction to link Shyu 2005 and 2008 (same trial). <br/> •   Added dementia to study characteristics table.<br/> •   Study and group ordering now consistent study characteristics and evaluation tables. | -->
<!-- | 2024-04-10 | •   GRADE for delirium now low based on concern for small study effects and potential for publication bias ([see asymmetric funnel plot examining small study effects](kq1.html#fig-rct_funnel)). <br/> •   For manuscript [balance table](balance_tables.html#expanded-preoperative-evaluation), omitted results from non-randomized studies except for delayed neurocognitive recovery and patient satisfaction. However, note that for hospital and 30-day mortality the GRADE was higher for nonrandomized studies. <br/> •   Updated [table of evaluation and intervention components](kq1.html#evaluationintervention-components) to include discussion of expectations and palliative care discussion, as well as haloperidol for delirium prophylaxis. Replaced changed for clarity. <br/> •   Added [table of intervention frequency](kq1.html#intervention-frequency). <br/> •   Physical function pooled removed period subgroups; arranged by time in forest plots and added figures showing limited change over time [for RCTs](kq1.html#fig-adl-time) and [nonrandomized studies](kq1.html#fig-adl-time_nrsi). <br/> •   Consistent with other TIVA versus inhaled anesthesia, adverse event results were combined (naïvely) from randomized and nonrandomized designs when the GRADE from randomized clinical trials would have been low or very low. <br/> •   Deleted pooled pulmonary complications owing to inconsistent outcome definitions. <br/> •   Reorganized discharge location pooled results to show randomized and nonrandomized designs. <br/> •   Added funnel plots, forest plots of risk differences, and baseline risk vs. risk ratio. <br/> •   Added [sensitivity analysis for pooled odds ratio to forest plot footnote](kq1.html#t-santabalbina2019_sensitivity_delirium) from nonrandomized studies to include the adjusted result from Tarazona-Santabalbina 2019 (little difference from unadjusted). -->

<!-- <caption_mg> Outcomes reported in additional publications (not necessarily unique studies).  </caption_mg> -->
...
<!-- expanded_dat <- readxl::read_excel("data/balance_tables_2023-09-14_mac_mg.xlsx", sheet = "ExpandedPreop", range = "A4:R26")[, c(2:4, 10, 12, 13, 15:18)] |> -->
...
<!--     tab_footnote("Error in transcribing confidence interval.", locations = cells_body(columns = prior_est, rows = prior_est %in% c("1.02 (0.12–8.46)"))) |>  -->
<!--     tab_footnote("Difference in confidence intervals TBD.", locations = cells_body(columns = prior_est, rows = prior_est %in% c("0.77 (0.30–1.64)"))) |>  -->
<!--   tab_footnote("Although the number of studies limits the ability to assess small study effects, reviewing funnel plot suggests that possibility. Discussed with task force and decision made to downgrade to low.", locations = cells_body(columns = grade_change, rows = outcome == "Delirium incidence")) -->
...
<!-- expanded_dat <- readxl::read_excel("data/balance_tables_2023-09-14_mac_mg.xlsx", sheet = "ExpandedPreop", range = "A36:R47")[, c(2:4, 10, 12, 13, 15:18)] |> -->
<!--     remove_empty(which = "cols") |> -->
<!--     clean_names() |> -->
<!--   mutate( -->
<!--     across(everything(), ~ replace(.x, is.na(.x), "")), -->
...
<!--   tab_style(style = cell_text(align = "center"), locations = cells_body(columns = c(rct, nrsi, rct_new, nrsi_new))) |> -->
<!--   tab_style(style = list(cell_text(color = riskdiff_color)), locations = cells_body(columns = c(estimate_95_percent_ci, measure, prior_est), rows = str_detect(measure, "RD"))) |> -->
<!--   tab_footnote("RCT: randomized controlled trial; NRSI: non-randomized study of intervention; GRADE: Grading of Recommendations Assessment, Development and Evaluation; Est: effect estimate; CI: confidence interval; RR: risk ratio; RD: risk difference.")  -->

<!--     # tab_footnote("Revised estimate from raw data; still approximate owing to pruning in those studies using propensity matching but more consistent with when a risk ratio was used.", locations = cells_body(columns = prior_est, rows = prior_est %in% c("-1.9 (3.0 to -0.7)", "0.62 (-1.23 to 3.49)", "-0.05 (-0.61 to 0.57)", "-7.1 (-21.8 to 21.8", "0.33 (0.14 to 0.57)", "-0.53 (-0.82 to -0.20)")))  -->
...
<font size = 4> `r table_ref()` Publications reporting dichotomous or count outcomes (not necessarily unique studies). </font>
...
<font size = 4> `r table_ref()` Publications reporting likert or ordinal outcomes (not necessarily unique studies). </font>
...
studychar_tab |>
  ...
  cols_hide(c(refid, arm_id, study, year, design_f_lab, surgs_single, pacu_screen, cog_imp_n, cog_imp, design_f_abbrev)) |>
  cols_label(
    study_l          = "Study",
    arm_n            = "N ",
    arm              = "Arm",
...
# components
component_exp <- Hmisc::Cs(refid, year, study, study_l, arm_id, arm_kq1_eval, kq1_eras, gen_comp, gen_multi, gen_comorbid, gen_nutrition, kq1_frail, kq1_cog, kq1_phys_func, kq1_psych, pt_nutr, pt_mobility, pt_surgery, pt_social, pt_expect, pt_palliative, staff_comorbid, staff_inappmed, staff_geriatrician, staff_multidisc, staff_supplement, staff_treatplan, staff_haloperidol, pacu_screen) 

component_std <- Hmisc::Cs(refid, year, study, study_l, arm_id, arm_kq1_eval, kq1_eras, gen_comp_std, gen_multi_std, gen_comorbid_std, gen_nutrition_std, gen_other_std, kq1_frail_std, kq1_cog_std, kq1_phys_func_std, kq1_psych_std, pt_nutr_std, pt_mobility_std, pt_surgery_std, pt_social_std, pt_expect_std, pt_palliative_std, staff_comorbid_std, staff_inappmed_std, staff_geriatrician_std, staff_multidisc_std, staff_supplement_std, staff_treatplan_std, staff_haloperidol_std, pacu_screen)

## expanded_dat ####
expanded_dat <- study_arm_dat |>
  filter(refid %in% kq1_refid) |>
  filter(!refid %in% c(1263, 17155)) |> # Hempenius 2016, Shyu 2008 second publication of same study
  mutate(
    pacu_screen = ifelse(pacu_screen, "kq1_pacu_screen", NA),
...
standard_dat <- study_arm_dat |>
  ...
  mutate(
        pacu_screen = ifelse(pacu_screen, "kq1_pacu_screen", NA),
        staff_haloperidol_std = NA_character_,
  ) |> 
  select(all_of(component_std)) |>
  ...
  mutate(
    arm = ifelse(arm_kq1_eval == "eval_expand", "Exp", "Std"),
    across(gen_comp:pacu_screen, ~ notna_to_x(.x, symbol = "STD")),
    across(
      gen_multi:kq1_psych,
      ~ case_when(
        !is.na(gen_comp) ~ NA,
  ...
  filter(if_any(kq1_eras:pacu_screen, ~ !is.na(.x))) |>
  filter(arm == "Std") |>
  slice(1, .by = study)
...
expanded_combined <- bind_rows(expanded_dat, standard_dat) |> 
  ...
  group_by(study) |>
  mutate(
    across(gen_comp:pacu_screen, ~ paste0(.x, dplyr::lag(.x))),
    across(gen_comp:pacu_screen, ~ str_remove_all(.x, "NA")),
    across(gen_comp:pacu_screen, ~ na_if(.x, ""))
  ) |>
  ungroup() |>
  filter(arm_id > 1) |>
  mutate(
    # half circles
    across(gen_comp:pacu_screen, ~ ifelse(.x == "STD", " \U25D1", .x)),
    across(gen_comp:pacu_screen, ~ ifelse(.x == "EXPSTD", "<font size = 4.5>  \U25CF </font>", .x)),
    across(gen_comp:pacu_screen, ~ ifelse(.x == "EXP", " \U25D0", .x)),
    # squares and triangles
    # across(gen_comp:pacu_screen, ~ ifelse(.x == "STD", " \U25CB", .x)),
    # across(gen_comp:pacu_screen, ~ ifelse(.x == "EXPSTD", " \UFFED", .x)),
    # across(gen_comp:pacu_screen, ~ ifelse(.x == "EXP", " \U25CF", .x)),
    # pacu_screen = ifelse(pacu_screen, " \U25CF", NA)
  )

# "<font size = 3>  \U25CF </font>"

# expanded_combined |> tabyl(surg_design) |> pull(surg_design) |> toString()

expanded_combined |> 
  arrange(year, study, arm_id) |> 
  select(refid, study, study_l, arm_id, arm, surg_design, kq1_eras, gen_comp, gen_multi, gen_comorbid, gen_nutrition, kq1_frail, kq1_cog, kq1_phys_func, kq1_psych, pt_nutr, pt_mobility, pt_surgery, pt_social, pt_expect, pt_palliative, staff_comorbid, staff_inappmed, staff_geriatrician, staff_multidisc, staff_supplement, staff_treatplan, staff_haloperidol, pacu_screen) |> 
  ...
  gt_theme_mg() |>
  sub_missing(columns = everything(), missing_text = "") |>
  cols_hide(c(refid, study, arm)) |>
  fmt_markdown(columns = c(study_l, gen_comp:pacu_screen)) |>
  cols_label(
    study_l              = md("Study"),
    ...
    pt_expect            = md(vert_lab_fun("Eexpectations")),
    pt_palliative        = md(vert_lab_fun("Palliative care discuss")),
    staff_comorbid       = md(vert_lab_fun("Meds/comorbidities")),
    staff_inappmed       = md(vert_lab_fun("Meds Review")),
    ...
    staff_haloperidol    = md(vert_lab_fun("Haloperidol prophylaxis")),
  ...
  tab_spanner(label = "General", columns = c(gen_comp:gen_nutrition), level = 1) |>
  ...
  tab_spanner(label = "Patient Education", columns = c(pt_nutr:pt_palliative), level = 1) |>
  ...
  tab_style(style = list(cell_text(color = color_1)), locations = cells_column_labels(columns = c(gen_multi:gen_nutrition))) |> 
  tab_style(style = list(cell_text(color = color_2)), locations = cells_column_labels(columns = c(kq1_frail:kq1_psych))) |> 
  tab_style(style = list(cell_text(color = color_3)), locations = cells_column_labels(columns = c(pt_nutr:pt_palliative))) |> 
  tab_style(style = list(cell_text(color = color_4)), locations = cells_column_labels(columns = c(staff_comorbid:pacu_screen))) |> 
  tab_style(style = list(cell_text(color = color_1)), locations = cells_body(columns = c(gen_multi:gen_nutrition), rows = everything())) |> 
  tab_style(style = list(cell_text(color = color_2)), locations = cells_body(columns = c(kq1_frail:kq1_psych), rows = everything())) |> 
  tab_style(style = list(cell_text(color = color_3)), locations = cells_body(columns = c(pt_nutr:pt_palliative), rows = everything())) |> 
  ...
  tab_footnote(md("ERAS: enhanced recovery after surgery; CGA: comprehensive geriatric assessment; GI: gastrointestinal; Abd: abdominal; Suppl: supplement;  \U25D0 expanded evaluation only; \U25CF in expanded and standard evaluation arms; \U25D1 in standard evaluation arms only.")) |> 
  tab_footnote("When a comprehensive geriatric assessment (CGA) was conducted, all components were presumed included. ", locations = cells_column_spanners(spanners = c("General", "Targeted")), placement = "right") |> 
  tab_footnote("Selective comprehensive geriatric assessment in standard arm.", locations = cells_body(columns = c(study_l), rows = study == "Giannotti 2022")) 
...
## components for exploratory nma ####
...
#   arrange(year, study, arm_id) |> 
#   select(-kq1_eras) |> 
# mutate(across(everything(), ~ str_remove(.x, "_gen")))

```

### Intervention Frequency

<caption_mg> `r table_ref()` Intervention components following expanded screening.  </caption_mg>
...
interventions_dat <- study_arm_dat |>
  ...
  filter(arm == "Exp") |>
  select(pacu_screen, staff_comorbid, staff_inappmed, staff_geriatrician, staff_multidisc, staff_supplement, staff_treatplan, staff_haloperidol, pt_nutr, pt_mobility, pt_social, pt_expect, pt_surgery, pt_palliative) |>
  mutate(
    pacu_screen = pacu_screen * 1,
  ) |>
  mutate(
    across(staff_comorbid:pt_palliative, ~ ifelse(!is.na(.x), 1, 0)),
    pacu_screen = pacu_screen * 1,
  ) |>
  select(staff_comorbid, staff_inappmed, staff_geriatrician, staff_multidisc, staff_supplement, staff_treatplan, pacu_screen, staff_haloperidol, pt_nutr, pt_mobility, pt_social, pt_expect, pt_surgery, pt_palliative)
...
interventions_1 <- interventions_dat |>
  select(staff_multidisc, staff_treatplan, staff_geriatrician, staff_inappmed, staff_supplement, staff_comorbid, pacu_screen, staff_haloperidol) |>
  tbl_summary(
    label = list(
      staff_comorbid       = "Rx/comorbidities",
      staff_inappmed       = "Medication Review/Deprescribing",
      ...
      staff_haloperidol    = "Haloperidol Prophylaxis",
      # staff_other          = "Other",
...
interventions_2 <- interventions_dat |>
  select(pt_mobility, pt_nutr, pt_social, pt_expect, pt_surgery, pt_palliative) |>
  tbl_summary(
    label = list(
      pt_mobility          = "Physical activity",
      pt_nutr              = "Nutrition",
      pt_social            = "Social Ties",
      pt_expect            = "Surgery/recovery expectations",
      pt_surgery           = "Procedure",
      pt_palliative        = "Palliative care discussion"
      # pt_other             = "Other"
    ),
    type = list(
      ...
      pt_expect            ~ "dichotomous",
      pt_surgery           ~ "dichotomous",
      pt_palliative        ~ "dichotomous"
...
delirium_meta_rct <- metabin(event1, n1, event2, n2,
...
delirium_meta_rct_rd <- update(delirium_meta_rct, sm = "RD", method = "MH")

# soe_meta_result_rr_or(delirium_meta_rct, "RR")
# soe_meta_rd_rr(delirium_meta_rct, digits = 1, scale = 100)

# temp <- update(delirium_meta_rct, sm = "OR", subgroup = NULL)
...
delirium_meta_nrsi <- metabin(event1, n1, event2, n2,
...
delirium_nrsi_or_dat <- update(delirium_meta_nrsi, sm = "OR", method = "MH")
delirium_nrsi_or_dat <- tibble(
  study = delirium_nrsi_or_dat$studlab,
  te = delirium_nrsi_or_dat$TE,
  se = delirium_nrsi_or_dat$seTE) |>
  mutate(
    te = ifelse(study == "T-Santabalbina 2019", log(0.23), te),
    se = ifelse(study == "T-Santabalbina 2019", se_ln_ci_fun(0.1, 0.51), se)
  )

delirium_meta_nrsi_rd <- update(delirium_meta_nrsi, sm = "RD", method = "MH")
...
#   method.predict = "HK",
#   prediction = TRUE,
#   data = delirium_nrsi_or_dat
# )

# 2024-04-10
# Common effect model       0.5909 (0.4537, 0.7698) -3.90 < 0.0001
# Random effects model (HK) 0.5854 (0.3880, 0.8832) -3.08   0.0178
# Prediction interval              (0.2885, 1.1878) 

# update(delirium_meta_nrsi, sm = "OR", method = "inverse")
# 
# Common effect model       0.5790 (0.4507, 0.7439) -4.27 < 0.0001
# Random effects model      0.5835 (0.4043, 0.8424) -3.47   0.0104
# Prediction interval              (0.3033, 1.1227)    

# soe_meta_result_rr_or(delirium_meta_nrsi, "RR")
# soe_meta_rd_rr(delirium_meta_nrsi, digits = 1, scale = 100)
...
Too few studies to adequately examine small study effects, but funnel suggests potential exists.</foot_mg>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Delirium incidence RCTs — small study effects. </caption_mg> <a id="fig-rct_funnel"></a>

```{r kq1_delirium_rct_funnel} 
#| echo: false
#| include: false

# FIXME: delirium meta-analysis funnel plot
delirium_meta_or <- update(delirium_meta_rct, sm = "OR")

png("assets/kq1_delirium_or_rct_funnel.png", width = 8.5, height = 5.5, units = "in", res = 300)
par(mar = c(4, 4, 2, 1))
...
funnel(delirium_meta_or, pch = 20, xlim = c(0.3, 1.6), contour = c(0.9, 0.95, 0.99), col.contour = limit_colors, studlab = TRUE, pos.studlab = 4, fs.smlab = 6, ff.smlab = "italic")
legend(0.3, 0.02, c("0.1 > p > 0.05", "0.05 > p > 0.01", "< 0.01"), fill = c("#AAB7B8", "#D5DBDB", "#F4F6F6"), bty = "n")
dev.off()

```

![](assets/kq1_delirium_or_rct_funnel.png){width="70%"}
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Delirium incidence in randomized clinical trials — risk difference per 100. </caption_mg> <a id="kq4_delirium_risk-diff"></a>

```{r kq1_delirium_meta_forest_rd}
#| echo: false
#| include: false
...
png("assets/kq1_delirium_meta_forest_rd.png", width = 9.84, height = 3.6, units = "in", res = 300)
...
dev.off()
```
![](assets/kq1_delirium_meta_forest_rd.png){fig.align="left" width="73%"}
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Control arm/baseline risk (standard evaluation) and risk ratios. </caption_mg> <a id="bubble-plot"></a>
...
png("assets/kq1_delirium_meta_bubble.png", width = 8, height = 6, units = "in", res = 300)
par(bty = "n", xaxt = "s", yaxt = "s", mar = c(4, 4, 2, 1))
metafor::regplot(fitted_meta,
  xlab = "Standard preoperative evaluation (baseline risk)",
  ylab = "RR (risk ratio)", 
  xlim = c(0.1, 0.6),
  at = log(c(0.2, 0.4, 0.6, 0.77, 1, 2)),
  # predlim = c(0, 0.30),
  atransf = exp,
  labsize = 0.7, 
  # predlim = c(0, 0.445),
...
plotrix::ablineclip(log(0.77), 0, col = "red", lty = 2, lwd = 0.75, x1 = 0.13, x2 = 0.53)
dev.off()
...
![](assets/kq1_delirium_meta_bubble.png){width="75%"}
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Delirium incidence summary risk of bias in RCTs (weighted). </caption_mg>

```{r rob2_summary_rct_delirium, fig.height = 3, out.width = "60%"}
...
<font size = 4> `r figure_ref()` Delirium incidence following expanded compared with standard preoperative evaluation in nonrandomized studies. </font>
...
<a id="t-santabalbina2019_sensitivity_delirium"></a>

<foot_mg> RR: risk ratio; D1: Bias due to confounding; D2: Bias in selection of participants into the study; D3: Bias in classification of interventions; D4: Bias due to deviations from intended interventions; D5: Bias due to missing data; D6: Bias in measurement of outcomes; D7: Bias in selection of reported results; All: overall risk of bias.<br/> Risk of bias ratings: low ++, moderate +, serious -, critical - - ; NI: no information.<br/> Including Vochteloo 2011 assuming effectively equal numbers of participants studied each year, obtained RR 0.70 (95% CI, 0.53-0.95; PI, 0.35–1.42). <br/> Pooling odds ratios using the adjusted result from Tarazona-Santabalbina 2019 yielded an OR 0.58 (95% CI, 0.39–0.88) versus using including the unadjusted data OR 0.58 (95%, 0.40–0.84).

</foot_mg>

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Delirium incidence nonrandomized studies — small study effects. </caption_mg>

```{r kq1_delirium_nrsi_funnel_limit} 
#| echo: false
#| include: false

# FIXME: delirium meta-analysis funnel plot
delirium_meta_or <- update(delirium_meta_nrsi, sm = "OR")

png("assets/kq1_delirium_or_nrsi_funnel.png", width = 8.5, height = 5.5, units = "in", res = 300)
par(mar = c(4, 4, 2, 1))
limit_colors <- c("#AAB7B8", "#D5DBDB", "#F4F6F6")
# text(1.2, 0.05, labels = "Adjusted estimate", col = "darkgray", pos = 3, cex = .9)
par(bty = "n", xaxt = "s", yaxt = "s")
...
legend(0.15, 0.02, c("0.1 > p > 0.05", "0.05 > p > 0.01", "< 0.01"), fill = c("#AAB7B8", "#D5DBDB", "#F4F6F6"), bty = "n")
dev.off()

```

![](assets/kq1_delirium_or_nrsi_funnel.png){width="70%"}
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Delirium incidence in nonrandomized studies — risk difference per 100. </caption_mg> <a id="kq4_delirium_risk-diff"></a>

```{r kq1_delirium_meta_nrsi_forest_rd}
#| echo: false
#| include: false

# get scaled tau
delirium_meta_nrsi_rd$tau <- delirium_meta_nrsi_rd$tau * 100

png("assets/kq1_delirium_meta_nrsi_forest_rd.png", width = 11.42, height = 4, units = "in", res = 300)
forest(delirium_meta_nrsi_rd,
  allstudies = TRUE,
  common = TRUE,
  label.e = "TIVA   ",
  label.c = "Inhaled   ",
  rightcols = c("effect", "ci", "surg_f", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "Overall"),
  rightlabs = c("RD", "(95% CI)", "Surgery", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "All"),
  just.addcols.right = "left",
  digits = 1,
  print.tau2 = FALSE,
  print.tau = TRUE,
...
![](assets/kq1_delirium_meta_nrsi_forest_rd.png){fig.align="left" width="73%"}

<foot_mg> RR: risk ratio; D1: Bias due to confounding; D2: Bias in selection of participants into the study; D3: Bias in classification of interventions; D4: Bias due to deviations from intended interventions; D5: Bias due to missing data; D6: Bias in measurement of outcomes; D7: Bias in selection of reported results; All: overall risk of bias.<br/> Risk of bias ratings: low ++, moderate +, serious -, critical - - ; NI: no information. </foot_mg>

:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Control arm/baseline risk (standard evaluation) and risk ratios — nonrandomized designs. </caption_mg> <a id="nrsi-bubble-plot"></a>
```{r bubble_plot_nrsi_delirium}
#| echo: false
#| include: false

temp <- bubble_plot_data(delirium_meta_nrsi) 

fitted_meta <- metafor::rma(yi = yi_meta, sei = sei_meta, mods = ~ control_event_rate, slab = slab, data = temp)

png("assets/kq1_delirium_meta_nrsi_bubble.png", width = 8, height = 6, units = "in", res = 300)
par(bty = "n", xaxt = "s", yaxt = "s", mar = c(4, 4, 2, 1))
metafor::regplot(fitted_meta,
  xlab = "Standard preoperative evaluation (baseline risk)",
  ylab = "RR (risk ratio)", 
  xlim = c(0.098, 0.38),
  at = log(c(0.2, 0.4, 0.64, 0.8, 1, 2)),
  # predlim = c(0, 0.30),
  atransf = exp,
  labsize = 0.7, 
  # predlim = c(0, 0.445),
...
plotrix::ablineclip(log(0.64), 0, col = "red", lty = 2, lwd = 0.75, x1 = 0.11, x2 = 0.35)
dev.off()
...
![](assets/kq1_delirium_meta_nrsi_bubble.png){width="75%"}

:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Delirium incidence summary risk of bias in nonrandomized studies (weighted). </caption_mg>

```{r robinsi_summary_nrsi_delirium, fig.height = 3, out.width = "60%"}
rob_summary_meta_weighted_nrsi_fun(delirium_meta_nrsi)
# rob_summary_fun(delirium_meta_dat$refid)
```
:::
...
<font size = 4> `r table_ref()` Neurocognitive disorder <30 days incidence and ascertainment (expanded versus standard preoperative evaluation; randomized and nonrandomized designs). </font>
...
dncr_dat |>
  ...
  mutate(design_surg = str_replace(design_surg, "Various", "Various Procedures")) |>
  ...
  tab_style(style = list(cell_text(color = "#A93226")), locations = cells_body(columns = c(arm_n:pre_mmse, neurocog_nper), rows = str_detect(arm, "Exp"))) |>
...
<caption_mg> No studies </caption_mg>
...
adl_likert_dat <- likert_dat |> 
  ...
  mutate(
    ...
    adl_scale = case_when(
      ...
      study == "Smoor 2023" ~ "SF-12 physical",
    )
  ) |> 
  filter(!is.na(l_adl)) |> 
...
adl_scales_dat <- tribble(
  ...
  "Smoor 2023", "SF-12 Physical", "0→100"
)
...
<a id="kq1_function_rct"></a>

#### Randomized

<font size = 4> `r figure_ref()` Pooled results for physical functional status for expanded versus standard preoperative evaluation from randomized clinical trials (higher is better). </font>
...
png("assets/kq1_adl_forest.png", width = 9.78, height = 3.6, units = "in", res = 300)
forest(adl_rct_meta,
  common = FALSE,
  label.e = "Expanded",
  label.c = "Standard",
...
# temp_refid <- study_char_dat |> 
#   filter(study %in% adl_meta_lte180_dat$study & design_f == "rct") |> 
#   pull(refid)

# rob2_traffic_light_refid(temp_refid)
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` SMDs over time in randomized clinical trials. </caption_mg> <a id="fig-adl-time"></a>
...
<a id="kq1_function_nrsi"></a>

<foot_mg>    </foot_mg> 

#### Nonrandomized

<font size = 4> `r figure_ref()` Pooled results for physical functional status for expanded versus standard preoperative evaluation from nonrandomized studies (higher is better). </font>

```{r exp_std_nrsi_forest}
#| include: false
#| echo: false

png("assets/kq1_adl_nrsi_forest.png", width = 9.83, height = 3.4, units = "in", res = 300)
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` SMDs over time in nonrandomized studies. </caption_mg> <a id="fig-adl-time_nrsi"></a>

```{r bubble_plot_nrsi_function}
#| include: false
#| echo: false

temp_nrsi <- tibble(with(adl_nrsi_meta, data.frame(yi_meta = TE, vi_meta = seTE, study = studlab, time = data$time[subset]))) |> 
  mutate(study = str_replace(study, "\\*", "")) 

fitted_meta <- metafor::rma.mv(yi_meta, vi_meta, random = ~ 1 | study, mods = ~ time,  slab = study, data = temp_nrsi)

png("assets/kq1_function_bubble_nrsi.png", width = 7, height = 5, units = "in", res = 300)
par(bty = "n", xaxt = "s", yaxt = "s", mar = c(4, 4, 2, 1))
metafor::regplot(fitted_meta,
  xlab = "Days",
  ylab = "SMD", 
...
rm(temp_nrsi, fitted_meta)
```

![](assets/kq1_function_bubble_nrsi.png){width="50%"}
...
<font size = 4> `r table_ref()` Complications reported in expanded versus standard preoperative evaluation --- cardiac, pulmonary, and acute kidney injury (randomized and nonrandomized designs). </font>
...
adverse_events_gt_dat |> 
  ...
  gt(id = "one") |>
  row_group_order(groups = c("MI – Randomized Clinical Trial", "MI – Before-After/Time Series", "Cardiac Arrest – Randomized Clinical Trial", "Cardiac Arrest – Before-After/Time Series", "Other Cardiac – Randomized Clinical Trial", "Other Cardiac – Before-After/Time Series", "Acute Kidney Injury – Randomized Clinical Trial", "Acute Kidney Injury – Before-After/Time Series", "Acute Kidney Injury – Before-After/Time Series", "Pulmonary – Randomized Clinical Trial", "Pulmonary – Retrospective Cohort", "Pneumonia – Randomized Clinical Trial", "Pneumonia – Before-After/Time Series", "Respiratory Failure – Before-After/Time Series", "Ventilator >48hr – Before-After/Time Series", "PE – Before-After/Time Series")) |>
...
complication_summary_rr_or <- function(compl_label){
  temp <- adverse_meta_wide_dat |> 
    ...
    filter(n_Exp + n_Std != 0) |>
    summarize(
      n_studies = n_distinct(refid),
      n_Exp_events = sum(n_Exp),
      n_Std_events = sum(n_Std),
      n_Exp = sum(arm_n_Exp),
      n_Std = sum(arm_n_Std),
      n_patients = sum(arm_n_Exp) + sum(arm_n_Std),
    ) |> 
    unlist() |> 
    toString() 
    # str_replace_all(",", "")
...
designs_summary <- function(temp_meta){
  study_char_dat |> 
    filter(refid %in% temp_meta$data$refid) |> 
    select(refid, study, design) 
}

```

<a id="fig-myocardial_infarction"></a>
...
temp_meta <- complication_meta_fun("MI", use_hakn = TRUE, predictive = TRUE)# complication_forest_fun(temp_meta, use_predictive = TRUE, text.addline1 = "*Randomized clinical trial", text.addline2 = risk_diff_meta_rr())
complication_forest_fun(temp_meta, use_predictive = TRUE, text.addline1 = "*Randomized clinical trial", text.addline2 = rd_ci_from_meta(temp_meta, pscale = 1000, digits = 1))
dev.off()

# meta_rob_traffic_light_refid(temp_meta)
# complication_summary_rr_or("MI")
# soe_meta_result_rr_or(temp_meta, "RR")
# soe_meta_rd_rr(temp_meta, digits = 1)
...
<a id="fig-cardiac_arrest"></a>

#### Cardiac Arrest

<foot_mg>    </foot_mg>

<font size = 4> `r figure_ref()` Risk ratio for cardiac arrest (randomized and nonrandomized designs). </font>

```{r complication_cardiac_arrest}
#| echo: false
#| include: false

png("assets/kq1_complication_cardiac_arrest.png", width = 8.87, height = 3.0, units = "in", res = 300)
temp_meta <- complication_meta_fun("Cardiac Arrest", use_hakn = FALSE, predictive = FALSE, random = FALSE)
complication_forest_fun(temp_meta, use_predictive = FALSE, text.addline1 = "*Randomized clinical trial", text.addline2 = rd_ci_from_meta(temp_meta, pscale = 1000, digits = 1))
dev.off()
...
![](assets/kq1_complication_cardiac_arrest){fig.align="left" width="66%"}
...
png("assets/kq1_complication_pe.png", width = 9.22, height = 3.4, units = "in", res = 300)
temp_meta <- complication_meta_fun("PE", use_hakn = FALSE, predictive = TRUE)
complication_forest_fun(temp_meta, use_predictive = TRUE, text.addline1 = rd_ci_from_meta(temp_meta, pscale = 1000, digits = 1))
dev.off()

# meta_rob_traffic_light_refid(temp_meta)
# complication_summary_rr_or("PE")
# soe_meta_result_rr_or(temp_meta, "RR")
# soe_meta_rd_rr(temp_meta, digits = 1)
# designs_summary(temp_meta)
...
los_refid <- sort(unique(los_tab$refid))
...
png("assets/kq1_los_meta.png", width = 9.55, height = 5.83, units = "in", res = 300)
forest(los_meta,
  weight.study = "random",
  # common =  FALSE,
  label.e = "Expanded ",
...
<font size = 4> `r figure_ref()` Mean difference in lengths of stay in nonrandomized studies. </font>
...
png("assets/kq1_dc_loc_meta.png", width = 10.19, height = 6.4, units = "in", res = 300)
forest(dc_loc_meta,
  allstudies = TRUE,
  common = TRUE,
  label.e = "Expanded",
...
<a id="fig-mortality_hosp_30d_rct"></a>
...
mortality_rct_meta_1yr_dat <- mortality_dat |>
  filter(str_detect(design_f_lab, "Rand")) |> 
...
pairwise_rct_mort_1yr_dat <- pairwise(
  treat = arm,
  event = mort_n,
  n = arm_n,
  studlab = study,
  allstudies = TRUE,
  data = mortality_rct_meta_1yr_dat,
  sm = "RR",
  keep.all.comparisons = TRUE
)
...
forest(mort_rct_meta,
  ...
  xlab = "Favors: Expanded                         Standard        ",
  addrows.below.overall = 4,
  text.addline2 = rd_ci_from_meta(mort_rct_meta)
)
dev.off()

# PI: (0.0067, 157.1841) 
# soe_meta_result_rr_or(mort_rct_meta, "RR")
# soe_meta_rd_rr(mort_rct_meta, digits = 1)

```
![](assets/kq1_mort_rct.png){fig.align="left" width="63%"}

<a id="fig-mort_rct_1yr"></a>

<br/>

<foot_mg>    </foot_mg> 

<font size = 4> `r figure_ref()` Risk ratio for 1-year mortality in randomized clinical trials. </font>

```{r mort_rct_1yr_meta}
#| echo: false
#| include: false

mort_meta_dat <- pairwise_rct_mort_1yr_dat |>
  mutate(year = str_extract(study, "\\d{4}")) |>
  left_join(surgs |> select(refid, surgs_single), by = "refid") |>
  select(study, year, event2, n2, event1, n1, surgs, mort_period, refid)
...
mort_rct_1yr_meta <- metabin(event2, n2, event1, n1,
  data = mort_meta_dat,
  studlab = study,
  sm = "RR",
  method = "MH",
...
png("assets/kq1_mort_rct_1yr.png", width = 8.68, height = 2.6, units = "in", res = 300)
forest(mort_rct_1yr_meta,
  allstudies = TRUE,
  random = FALSE,
  label.e = "Expanded  ",
  label.c = "Standard  ",
  ...
  xlab = "Favors: Expanded                         Standard        ",
  addrows.below.overall = 4,
  text.addline2 = rd_ci_from_meta(mort_rct_1yr_meta)
)
dev.off()

# soe_meta_result_rr_or(mort_rct_1yr_meta, "RR")
# soe_meta_rd_rr(mort_rct_1yr_meta, digits = 1)

```
![](assets/kq1_mort_rct_1yr.png){fig.align="left" width="65%"}

<a id="fig-mort_nrsi"></a>

<foot_mg>    </foot_mg> 

#### Nonrandomized

<font size = 4> `r figure_ref()` Risk ratio for hospital or 30-day mortality in nonrandomized studies. </font>
...
forest(mort_nrsi_meta,
  ...
  xlab = "Favors: Expanded                         Standard        ",
  addrows.below.overall = 4,
  text.addline2 = rd_ci_from_meta(mort_nrsi_meta)
)
dev.off()

# soe_meta_result_rr_or(mort_nrsi_meta, "RR")
# soe_meta_rd_rr(mort_nrsi_meta, digits = 1)

```
![](assets/kq1_mort_nrsi.png){fig.align="left" width="67%"}

<a id="fig-mort_nrsi_1yr"></a>

<br/>

<foot_mg>    </foot_mg> 

<font size = 4> `r figure_ref()` Risk ratio for 1-year mortality in nonrandomized studies. </font>

```{r mort_nrsi_1yr_meta}
#| echo: false
#| include: false
...
png("assets/kq1_mort_nrsi_1yr.png", width = 8.72, height = 3.6, units = "in", res = 300)
forest(mort_nrsi_1yr_meta,
  ...
  xlab = "Favors: Expanded                         Standard        ",
  addrows.below.overall = 4,
  text.addline2 = rd_ci_from_meta(mort_nrsi_1yr_meta)
)

dev.off()

# soe_meta_result_rr_or(mort_nrsi_1yr_meta, "RR")
# soe_meta_rd_rr(mort_nrsi_1yr_meta, digits = 1)

```

![](assets/kq1_mort_nrsi_1yr.png){fig.align="left" width="65%"}
