...
studychar_tab <- study_arm_dat |>
  ...
  mutate(
    arm = replace_na(arm, "Active Screening"),
...
studychar_tab |>
  ...
  tab_footnote(md("Exp: expanded preoperative evaluation; Std: standard preoperative evaluation; Active Screening: active delirium screening; NR: not reported")) |>
...
<font size = 4> `r table_ref()` Discharge location in studies comparing expanded preoperative assessments with standard preoperative assessments. </font>
...
# referent values for calculating rr and ci; rr_ci
disch_rr_ref <- dc_loc_dat |>
  select(refid, arm_id, disch_inst_n, arm_n) |>
  arrange(refid, arm_id) |>
  rename(ref_disch_n = disch_inst_n, ref_arm_n = arm_n) |>
  # group_by(refid) |>
  mutate(
    ref_arm_n = ifelse(row_number() > 1, NA, ref_arm_n),
    ref_disch_n = ifelse(row_number() > 1, NA, ref_disch_n),
    .by = refid
  ) |>
  fill(ref_arm_n, ref_disch_n) |>
  mutate(
    ref_arm_n = ifelse(row_number() == 1, NA, ref_arm_n),
    ref_disch_n = ifelse(row_number() == 1, NA, ref_disch_n),
    .by = refid
...
dc_loc_dat <- dc_loc_dat |> 
  left_join(disch_rr_ref, by = c("refid", "arm_id")) |> 
  arrange(study, arm_id) |> 
  mutate(rr_ci = ifelse(!is.na(ref_disch_n), rr_ci_fun(disch_inst_n, arm_n, ref_disch_n, ref_arm_n), NA)) |> 
