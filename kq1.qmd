...
| 2024-04-08 | Added table of intervention frequency. <br/> Physical function pooled removed period subgroups; arranged by time in forest plot and for pooled RCTs add a bubble plot. <br/> Consistent with other questions, adverse event results were combined (naïvely) from randomized and nonrandomized designs when the GRADE from randomized clinical trials would have been low or very low. <br/> Deleted pooled pulmonary complications owing lack of consistent outcome definitions. <br/> Reorganized discharge location pooled to display for randomized and nonrandomized designs. <br/> Omitted balance table results for body of text from non-randomized designs except for hospital and 30-day mortality (GRADE higher for nonrandomized). |
| 2024-04-08 | Changed GRADE for delirium to low based on possible small study effects with the potential for publication bias. |
...
# soe_meta_result_rr_or(delirium_rct_meta, "RR")
# soe_meta_rd_rr(delirium_rct_meta, digits = 1, scale = 100)
...
# soe_meta_result_rr_or(delirium_nrsi_meta, "RR")
# soe_meta_rd_rr(delirium_nrsi_meta, digits = 1, scale = 100)
...
#### <caption_mg> `r figure_ref()` Delirium incidence RCTs — small study effects. </caption_mg>
...
delirium_meta_or <- update(delirium_rct_meta, sm = "OR")
...
rob_summary_meta_weighted_fun(delirium_rct_meta)
...
# FIXME: delirium meta-analysis funnel plot
delirium_meta_or <- update(delirium_nrsi_meta, sm = "OR")
...
rob_summary_meta_weighted_nrsi_fun(delirium_nrsi_meta)
# rob_summary_fun(delirium_meta_dat$refid)
```
:::
...
<a id="fig-mortality_hosp_30d_nrsi"></a>
