...
soe_kq3.dat <- readxl::read_xlsx("data/soe_2023-04-27_local.xlsx", range = "A1:Z18", sheet = "soe_kq3_sp", col_types = col_types_soe) |>
  clean_names() |>
  mutate(
...
<font size = 4> `r table_ref()` Strength of evidence summaries --- regional versus general anesthesia. </font>
...
<font size = 4> Table `r table_n`. GRADE domains for strength of evidence ratings --- regional versus general anesthesia. </font>

```{r kq3Domain, eval = TRUE}
#| warning: true
#| include: true
#| echo: false
...
kq3_domain |> 
  ...
  mutate(space = "") |> 
  left_join(soe_kq3.dat |> select(compare_group, n), by = "n") |>
  # group_by(compare_group) |> 
  ...
  tab_footnote("Delayed neurocognitive recovery — 3 to 30 days postoperative.") |> 
  # tab_footnote("3 to 30 days postoperative.", locations = cells_body(columns = outcome, rows = str_detect(outcome, "Delayed"))) |> 
  tab_footnote(md(domain_footnotes))
```

<!-- ### Balance Table -->

<!-- ![](assets/regional_general_balance_2023-05-22.png){fig-align="left" width="6in"} -->
...
#| include: false
col_types_soe <- c("numeric", rep("text", 5), "numeric", rep("text", 19))
soe_kq4.dat <- readxl::read_xlsx("data/soe_2023-04-27_local.xlsx", range = "A1:Z23", sheet = "soe_kq4_sp", col_types = col_types_soe) |>
  filter(!n %in% c(4.52)) |>
  clean_names() |>
  mutate(
    n_s = paste0(n_studies, " <br/>(", format(n_pts, big.mark = ","), ")"),
...
soe_summary |>
  ...
  sub_values(values = c("NA <br/>(NA)"), replacement = "—") |>
  # tab_style(style = "vertical-align:top", locations = cells_body(columns = everything())) |> 
  tab_footnote("NRSI: non-randomized studies of interventions; Pts: patients; GRADE: Grades of Recommendation, Assessment, Development, and Evaluation; ACCF/AHA: American College of Cardiology Foundation/American Heart Association; RR: risk ratio.") |>
  tab_footnote(md(grade_foot), locations = cells_column_labels(columns = grade)) |>
  tab_footnote("A: high-quality evidence from more than 1 RCT; B-R: moderate-quality evidence from more than 1 RCT; B-NR: moderate-quality evidence from more than one observational study; C-LD: randomized or non-randomized observational or registry studies with limited data; C-EO: consensus of expert opinion.", locations = cells_column_labels(columns = acc)) 
...
<font size = 4> Table `r table_n`. GRADE domains for strength of evidence ratings --- TIVA versus inhaled anesthesia. </font>

```{r kq4Domain, eval = TRUE}
#| warning: true
#| include: true
#| echo: false
...
kq4_domain |>
  ...
  mutate(space = "") |>
  left_join(soe_kq4.dat |> select(compare_group, n), by = "n") |>
  # group_by(compare_group) |>
  select(outcome, design, space, n_s, rob, consist, direct, prec, other) |>
  gt(id = "one") |>
  cols_hide(n_s) |>
  ...
  # opt_footnote_marks(marks = "standard") |>
  tab_footnote("If not specified in footnotes, all references are to 95% confidence and prediction intervals.") |>
  tab_footnote("RCT: randomized clincal trial; NRSI: non-randomized studies of interventions; RR: risk ratio; OR: odds ratio.") |>
  tab_footnote("Delayed neurocognitive recovery — 3 to 30 days postoperative.") |>
  # tab_footnote("3 to 30 days postoperative.", locations = cells_body(columns = outcome, rows = str_detect(outcome, "Delayed"))) |>
  tab_footnote(md(domain_footnotes))
```
...
<!-- ![](assets/tiva-inhaled_balance_2023-05-22.png){fig-align="left" width="6in"} -->
...
col_types_soe <- c("numeric", rep("text", 5), "numeric", rep("text", 19))
soe_kq6.dat <- readxl::read_xlsx("data/soe_2023-04-27_local.xlsx", range = "A1:Z14", sheet = "soe_kq6", col_types = col_types_soe) |>
  clean_names() |>
  mutate(
    n_s = paste0(n_studies, " <br/>(", format(n_pts, big.mark = ","), ")"),
    n_s = paste0(n_studies, " <br/>(", n_pts, ")"),
...
<font size = 4> `r table_ref()` GRADE domains for strength of evidence ratinsg --- delirium prophylaxis. </font>
