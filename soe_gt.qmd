...
domain_footnote <- function(x) {
  x |>
    ...
    tab_footnote("RCT: randomized clinical trial; NRSI: non-randomized studies of interventions (inlcudes non-randomized trials, before-after/time series, and observational); RR: risk ratio; OR: odds ratio.") |>
...
soe_kq1_dat <- readxl::read_xlsx("data/soe_2023-05-22_local.xlsx", range = "A1:W29", sheet = "soe_kq1", col_types = col_types_soe) |>
  clean_names() |>
...
#| include: false
col_types_soe <- c("numeric", rep("text", 5), "numeric", rep("text", 16))
soe_kq3_dat <- readxl::read_xlsx("data/soe_2023-05-22_local.xlsx", range = "A1:W19", sheet = "soe_kq3", col_types = col_types_soe) |>
  clean_names() |>
  mutate(
...
#| include: false
col_types_soe <- c("numeric", rep("text", 5), "numeric", rep("text", 16))
soe_kq4_dat <- readxl::read_xlsx("data/soe_2023-05-22_local.xlsx", range = "A1:W22", sheet = "soe_kq4", col_types = col_types_soe) |>
  filter(!n %in% c(4.52)) |>
  clean_names() |>
  mutate(
    n_s = paste0(n_studies, " <br/>(", format(n_pts, big.mark = ","), ")"),
...
<font size = 4> `r table_ref()` GRADE domains for strength of evidence ratings --- delirium prophylaxis. </font>
