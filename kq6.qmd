...
<caption_mg> `r table_ref()` Publications reporting dichotomous or count outcomes (not unique studies).. </caption_mg>
...
<caption_mg> `r table_ref()` Publications reporting continuous outcomes (not unique studies). </caption_mg>

```{r cont_outcome_freq}
contin_freq_fun(contin_dat)
```

<caption_mg> `r table_ref()` Publications reporting likert or ordinal outcomes (not unique studies). </caption_mg>
...
delirium_meta_dat |>
  ...
  scale_colour_manual(values = c("#e41a1c", "#8B0000", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00", "#D4AC0D", "#a65628")) +
...
delirium_total_gt_fun("Dex") |>
  tab_footnote("RR: risk ratio; Dex: dexmedetomidine; Mid: midazolam; Prop: propofol; MS: morphine sulfate; Preg: pregabalin; Plac: placebo; DSM: Diagnostic and Statistical Manual of Mental Disorders; ICDSC: Intensive Care Delirium Screening Checklist; Psych: psychiatrist interview; Nu-DESC: Nursing Delirium Screening Scale; NS: not specified.") |>
  # tab_footnote(md("Mean <u>Med</u> (SD)[Range]{IQR}."), locations = cells_column_labels(columns = c(age_table, pre_mmse))) |>
...
<caption_mg> `r figure_ref()` Randomized clinical trials and comparators reporting delirium incidence. </caption_mg>
...
dex_plac_country_meta <- update(dex_plac_meta, subgroup = china_vs_other)
dex_plac_china_meta <- update(dex_plac_meta, subset = china_vs_other == "China", subgroup = NULL)
dex_plac_other_meta <- update(dex_plac_meta, subset = china_vs_other != "China", subgroup = NULL)
...
png("assets/kq6_dex_delirium_all.png", width = 9.81, height = 8.8, units = "in", res = 300)
dex_meta_forest(dex_plac_meta)
dev.off()
...
<foot_mg> Dex: dexmedetomidine; Plac: placebo; RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- . <br/> Meta-regression (China vs. Other), *I*<sup> 2</sup> = `r paste0(digit0(temp_metareg$I2), "%.")` <br/> \*Trials conducted in China. </foot_mg>
...
temp_limit_meta <- limitmeta(temp_meta, method.adjust = "beta0")
...
temp_limit_china_meta <- limitmeta(temp_china_meta, method.adjust = "beta0") # note RR 2024-04-15
temp_limit_china_meta <- limitmeta(temp_china_meta, method.adjust = "betalim") # note RR 2024-04-15


par(bty = "n", xaxt = "s", yaxt = "s")

funnel(temp_limit_china_meta, ylim = c(0.8, 0.0), xlim = c(0.1, 3), pch = 20, contour = c(0.9, 0.95, 0.99), col.contour = limit_colors, studlab = TRUE, pos.studlab = 4, fs.smlab = 6, ff.smlab = "italic")
...
<foot_mg> Estimates (risk ratio and 95% CI) from random effects models --- unadjusted `r paste0(digit2(exp(temp_limit_china_meta$TE.random)), " (", digit2(exp(temp_limit_china_meta$lower.random)), "–", digit2(exp(temp_limit_china_meta$upper.random)), ")")`; adjusted for small study effects: `r paste0(digit2(exp(temp_limit_china_meta$TE.adjust)), " (", digit2(exp(temp_limit_china_meta$lower.adjust)), "–", digit2(exp(temp_limit_china_meta$upper.adjust)), ")")`.<br/> `r paste0("Harbord test for small study effects P = ", formatC(temp_china_metabias$pval, digits = 4, format = "f"), ".")`<br/></foot_mg>
:::
...
temp_limit_meta <- limitmeta(temp_other_meta, method.adjust = "beta0") # note RR 2024-04-1511

limit_colors <- c("#AAB7B8", "#D5DBDB", "#F4F6F6")

par(bty = "n", xaxt = "s", yaxt = "s")

funnel(temp_limit_meta, ylim = c(0.9, 0.0), xlim = c(0.1, 5), pch = 20, contour = c(0.9, 0.95, 0.99), col.contour = limit_colors, studlab = TRUE, pos.studlab = 4, fs.smlab = 6, ff.smlab = "italic")
...
<foot_mg> Estimates (risk ratio and 95% CI) from random effects models --- unadjusted `r paste0(digit2(exp(temp_limit_meta$TE.random)), " (", digit2(exp(temp_limit_meta$lower.random)), "–", digit2(exp(temp_limit_meta$upper.random)), ")")`; adjusted for small study effects: `r paste0(digit2(exp(temp_limit_meta$TE.adjust)), " (", digit2(exp(temp_limit_meta$lower.adjust)), "–", digit2(exp(temp_limit_meta$upper.adjust)), ")")`.<br/> `r paste0("Harbord test for small study effects P = ", formatC(temp_metabias$pval, digits = 4, format = "f"), ".")`<br/></foot_mg>

<!-- # TODO: fix harbor test -->
<!-- # RESUME: 2024-04-22 -->
...
```{r meta_dat_postop_only, fig.width = 14, fig.height = 12.5, fig.align = "left"}
dex_plac_meta_postop <- update(dex_plac_meta, subgroup = postop_only)

dex_meta_forest(dex_plac_meta_postop)
...
```{r meta_postop_any, fig.width = 14, fig.height = 12.5, fig.align = "left"}
dex_plac_meta_postop_any <- update(dex_plac_meta, subgroup = postop_any)

dex_meta_forest(dex_plac_meta_postop_any)
...
```{r nma_dat_postop, fig.width = 14, fig.height = 17.3, fig.align = "left"}
dex_plac_meta_surgs <- update(dex_plac_meta, hakn = FALSE, subgroup = surg_f)

dex_meta_forest(dex_plac_meta_surgs)
...
```{r dex_meta_sens_incidence_prop, fig.width = 14, fig.height = 13.5, fig.align = "left"}
delirium_inc_pro_meta <- update(dex_plac_meta, subgroup = delirium_inc_prop, hakn = FALSE)

dex_meta_forest(delirium_inc_pro_meta)
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Control arm/baseline delirium risk and risk ratios. </caption_mg> <a id="dex-bubble-plot"></a>
```{r bubble_plot_delirium}
#| eval: false
#| echo: false
#| include: false

# RESUME: 2024-04-15 bubble plot
...
metafor::regplot(fitted_meta,
  xlab = "Placebo delirium event rate (baseline risk)",
  ylab = "RR (risk ratio)", 
  xlim = c(0.05, 0.7),
...
# abline(log(0.57), 0, col = "red", lty = 2, lwd = 0.75)
...
![](assets/kq6_delirium_meta_bubble.png){width="75%"}
...
```{r nma_dat_rob, fig.width = 14, fig.height = 13.5, fig.align = "left"}
rob_meta <- update(dex_plac_meta, subgroup = rob)

dex_meta_forest(rob_meta)
...
# par(oma = rep(0, 4), mar = c(2.5, .01, 0, .01))
...
<caption_mg> `r table_ref()` Delayed neurocognitive recovery incidence and ascertainment in trials of dexmedetomidine (includes only trials conducting cognitive testing at day 3 or later).</caption_mg>
...
dncr_dat |> 
  ...
  tab_footnote("American Psychiatric Association POCD criteria.", locations = cells_body(columns = c(scale_other_tf), rows = !is.na(scale_other_tf) & study == "Gao 2020")) 

```

### Ketamine

<caption_mg> `r table_ref()` Delayed neurocognitive recovery incidence and ascertainment in randomized clinical trials of ketamine (includes only trials conducting cognitive testing at day 3 or later).</caption_mg>
...
dncr_dat |> 
  ...
  row_group_order(groups = c("Various", "Ortho", "Cardiac", "GI/Abd")) |>
...
neurocog_meta_dat <- neurocog_allarms_meta_dat |>
  ...
  mutate(
    neurocog_total_n = case_when(
      ...
      study == "Wang 2020d" & arm_id == 2 ~ collapse_dichot(neurocog_allarms_meta_dat, "Wang 2020d", c(2, 3), neurocog_total_n),
      study == "Feng 2021" & arm_id == 2 ~ collapse_dichot(neurocog_allarms_meta_dat, "Feng 2021", c(2, 3), neurocog_total_n),
    ...
    arm_n = case_when(
      ...
      study == "Wang 2020d" & arm_id == 2 ~ collapse_dichot(neurocog_allarms_meta_dat, "Wang 2020d", c(2, 3), arm_n),
      study == "Feng 2021" & arm_id == 2 ~ collapse_dichot(neurocog_allarms_meta_dat, "Feng 2021", c(2, 3), arm_n),
      
      .default = arm_n
    ),
    arm = fct_collapse(arm, Plac = c("Plac", "None"))
  ) |>
  filter(!(study %in% c("Pan 2016") & arm_id %in% c(2, 3))) |>
  filter(!(study %in% c("Zhou 2019b") & arm_id %in% c(4))) |>
  filter(!(study %in% c("Zhao 2020") & arm_id %in% c(3, 4))) |>
  filter(!(study %in% c("Li 2021b") & arm_id %in% c(3, 4))) |>
  filter(!(study %in% c("Wang 2020d") & arm_id %in% c(3))) |>
  filter(!(study %in% c("Feng 2021") & arm_id %in% c(3))) |>
...
<caption_mg> `r table_ref()` Neurocognitive disorder and ascertainment. </caption_mg>
...
<caption_mg> `r figure_ref()`  Postoperative neurocognitive disorder. </caption_mg>

```{r fig_pooled, fig.height = 5, fig.width = 16, out.width = "60%", out.height = "60%", fig.align = 'center'}
...
# create meta analysis data set using pairwise 
...
forest(ncd_dex_meta,
  ...
  label.c = "Plac",
  rightcols = c("effect", "ci", "D1", "D2", "D3", "D4", "D5", "Overall"),
  rightlabs = c("RR", "(95% CI)", "D1", "D2", "D3", "D4", "D5", "Overall"),
  digits = 2,
  digits.tau2 = 1,
  overall.hetstat = TRUE,
  print.I2.ci = TRUE,
...
```{r complication_brady_meta, fig.width = 16, fig.height = 8, fig.align = "left"}
#| warning: false
...
```{r complication_brady_meta_rd, fig.width = 16, fig.height = 8, fig.align = "left"}
...
```{r complication_hypo_meta, fig.width = 16.5, fig.height = 8.75, fig.align = "left"}
#| warning: false 
# RESUME: hypotension
...
```{r complication_hypo_meta_rd, fig.width = 16, fig.height = 8.8, fig.align = "left"}
adverse_meta_rd <- update(adverse_meta, sm = "RD")
...
```{r complication_meta,  fig.width = 16, fig.height = 14, fig.align = "left"}
adverse_meta <- metabin(
  n_Dex, arm_n_Dex, n_Plac, arm_n_Plac,
...
```{r los_meta, fig.width = 16, fig.height = 10, fig.align = "left"}
...
```{r los_dex_meta, fig.width = 16, fig.height = 10, fig.align = "left"}
...
# update(los_dex_meta, subset = study != "Guo 2022")
# 
# par(bty = "n", xaxt = "s", yaxt = "s", mar = c(4, 4, 2, 1))
...
```{r los_mel_ram_meta, fig.width = 14, fig.height = 4.5, fig.align = "left"}
...
```{r dex_mort_meta, fig.dim = c(16, 8), fig.align = "center"}
...
```{r mel_ram_mort_meta, fig.dim = c(16, 7.5), fig.align = "center"}
# ```{r nma_dat, out.width = 50%, out.height = 100%}
