...
cardiac_meta_dat <- adverse_meta_dat |>
  ...
  mutate(
    n = case_when(
      study == "Zhao 2020" & arm_id == 2 ~ collapse_dichot(adverse_meta_dat, "Zhao 2020", c(2, 3, 4), n),
      study == "Li 2021b" & arm_id == 2 ~ collapse_dichot(adverse_meta_dat, "Li 2021b", c(2, 3, 4), n),
      .default = n
    ),
    arm_n = case_when(
      study == "Zhao 2020" & arm_id == 2 ~ collapse_dichot(adverse_meta_dat, "Zhao 2020", c(2, 3, 4), arm_n),
      study == "Li 2021b" & arm_id == 2 ~ collapse_dichot(adverse_meta_dat, "Li 2021b", c(2, 3, 4), arm_n),
      .default = arm_n
    ),
    arm = fct_collapse(arm, Plac = c("Plac", "None"))
  ) |>
...
netmeta_nma <- netmeta(
  ...
  sm = "RR",
  reference.group = "Plac",
  warn = FALSE # 2 dbl zero studies excluded
)
...
# function to select subset pairwise data
dex_cardiac_dat <- pairwise_dat |> 
    left_join(rob2_meta_dat, by = "refid") |> 
  filter(treat1 == "Plac" & treat2 == "Dex") 
```

```{r cardiac_comp_forest, warning = FALSE, fig.width = 14, fig.height = 4, fig.align = "left"}
#| echo: false
dex_cardiac_meta <- metabin(event2, n2, event1, n1,
  data = dex_cardiac_dat,
  studlab = study,
  # cluster = refid,
  sm = "RR",
  method = "MH",
...
# small study effects; here to have result of Harbord test
temp_meta <- dex_cardiac_meta 
temp_meta <- update(temp_meta, subgroup = NULL, sm = "OR")
temp_metabias <- metabias(temp_meta, method.bias = "Harbord", k.min = 10)

# summary(dex_plac_meta)
forest(dex_cardiac_meta,
  allstudies = TRUE,
  # random = FALSE,
  label.e = "Dex     ",
  label.c = "Plac     ",
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention with respect to composite cardiac compications  — small study effects. </font>

```{r dex_plac_cardiac_meta_funnel, fig.dim = c(7, 5.5), out.width = "70%", fig.align = "center"}
dex_cardiac_meta <- update(dex_cardiac_meta, sm = "OR", subgroup = NULL)
harbord <- metabias(dex_cardiac_meta, method.bias = "Harbord")$pval
par(mar = c(4, 4, 2, 1))
par(bty = "n", xaxt = "s", yaxt = "s")

funnel(dex_cardiac_meta,
