...
dichot_freq_fun(dichot_dat, "ADL: activities of daily living; NCR: neurocognitive recovery; QoR: quality of recovery; ")
```
...
likert_freq_fun(likert_dat, "ADL: activities of daily living; NCR: neurocognitive recovery; QoR: quality of recovery; ")
```
...
dex_by_arm <- drugs_dat |>
  ...
  select(study, year, refid, study_l_w_linked, arm_n, drug_recode, drug_recode_abbr, loading, maintain_range, postdose, pre:post, design_f_lab)
```
...
dex_by_arm |>
  arrange(year, study) |> 
  group_by(study_l_w_linked) |>
  mutate(study_l_w_linked = ifelse(row_number() > 1, "", study_l_w_linked),
         drug_recode = drug_recode_abbr
         ) |>
  ungroup() |>
  group_by(design_f_lab) |>
  gt(id = "one") |>
  cols_hide(c(year, study, refid, drug_recode_abbr)) |>
  cols_label(
    study_l_w_linked = "Study",
    arm_n = "N",
    drug_recode = "Comparator",
    loading = "Loading",
    maintain_range = md("Maintenance<br/>per hr"),
    postdose = md("Postop"),
    pre = "Pre",
    ind = "Ind",
    intr = "Intr",
  ...
  fmt_markdown(columns = c(study_l_w_linked, pre, ind, intr, post)) |> 
  tab_style(
    style = cell_text(align = "center"),
    locations = cells_column_labels(
      columns = c(drug_recode, loading:postdose)
    )
  ) |>
  tab_style(
    style = cell_text(align = "center"),
    locations = cells_body(
      columns = c(drug_recode, loading, maintain_range, postdose, pre:post)
    )
  ) |>
  tab_style(
    style = cell_text(align = "left"),
    locations = cells_body(
      columns = c(study_l_w_linked, pre:post)
    )
  ) |>
  tab_style(
    style = list(
      cell_text(color = "#A93226")
    ),
    locations = cells_body(
      columns = c(arm_n, drug_recode),
      rows = str_detect(drug_recode, "Dex")
    )
  ) |>
  tab_style(
  ...
  tab_footnote("Dex: Dexmedetomidine; Ket: Ketamine; Mel: Melatonin; Ram: Ramelteon; Uli: Ulinastatin; Hal: Haloperidol; Mid: Midazolam; Mid/Prop: Midazolam/propofol; Mid/Seve: Midazolam/sevoflurane; MS: Morphine; Preg: Pregabalin; Prop: Propofol; Rhyth: Rhythim; Sufen: Sufentanil; Plac: placebo; Prep: preoperative; Ind: induction; Intr: intraoperative; Post: postoperative.") |>
  tab_footnote("Range if reported in parentheses.",
    locations = cells_column_labels(columns = maintain_range)
  )
...
ket_by_arm <- drugs_dat |>
  ...
  # dosing
  arrange(year, refid, desc(drug)) |>
  select(study_l_w_linked, arm_n, drug, dose, dose_iv, time_admin, pre:postop, year, study, refid, design_f_lab)
...
ket_by_arm |>
  ...
  group_by(design_f_lab) |> 
  gt(id = "one") |>
  cols_hide(c(year, study, refid, time_admin)) |>
  cols_label(
    study_l = "Study",
    arm_n = "N",
    drug = "Comparator",
  ...
  fmt_markdown(columns = c(study_l, pre:postop)) |>
  tab_style(
    style = cell_text(align = "left"),
    locations = cells_column_labels(
  ...
  tab_style(
    style = cell_text(align = "left"),
    locations = cells_body(
      columns = c(study_l, drug, dose, dose_iv, pre:postop)
    )
  ) |>
  tab_style(
    style = list(
  ...
  tab_footnote(
    footnote = "Time administered not reported.",
    locations = cells_body(
      columns = pre,
      rows = arm_n == 38 & drug == "Ketamine")
  ) |>
  tab_footnote("Preop: preoperative; Induct: induction; Intraop: intraoperative; Post: postoperative; Unspec: unspecified.")
```
...
mel_ram_by_arm |>
  ...
  tab_style(
    style = cell_text(align = "left"),
    locations = cells_body(
      columns = c(study_l, drug, dose, pre, postop)
    )
  ) |>
  tab_style(
    style = list(
...
<font size = 4> `r table_ref()` Delirium incidence over defined lengths of hospitalization in randomized clinical trials of dexmedetomidine. </font>

```{r delirium_total_tab}
# CODE: delirium incidence
# total 
delirium_total_tab <- dichot_dat |>
  # select(refid, study, matches("^deli")) |>
  # select(refid, study, arm_n, matches("*time"), matches("delitotal")) |> 
...
delirium_total_tab |>
  # mutate(
  #   arm_id = ifelse(refid == 13734 & drug_recode_abbr == "Dex", 2, arm_id),
  #   arm_id = ifelse(refid == 13734 & drug_recode_abbr == "Rhyth", 1, arm_id),
  #   arm_id = ifelse(study == "Liu 2016 (no MCI)" & drug_recode_abbr == "Dex", 2, arm_id),
  #   arm_id = ifelse(study == "Liu 2016 (no MCI)" & drug_recode_abbr == "Plac", 1, arm_id),
  # ) |> 
