...
```{r rob2_summary, fig.height = 3, out.width = "50%"}
rob_temp_dat <- rob2_reconciled_dat |> 
  filter(refid %in% dex_refid) |> 
  select(-refid)

rob_summary(rob_temp_dat, tool = "ROB2")
```
:::
...
```{r rob2_summary_ket, fig.height = 3, out.width = "50%"}
rob_temp_dat <- rob2_reconciled_dat |> 
  filter(refid %in% ket_refid) |> 
  select(-refid)
...
```{r rob2_summary_mel_ram, fig.height = 3, out.width = "50%"}
rob_temp_dat <- rob2_reconciled_dat |> 
  filter(refid %in% mel_ram_refid) |> 
  select(-refid)
...
<font size = 4> `r figure_ref()` Risk of bias assessment for randomized controlled trials. </font>

```{r traffic, out.width = "70%", fig.height = 16}
rob_traffic_light(rob2_reconciled_dat |> select(-refid), psize = 4, tool = "ROB2")
```
