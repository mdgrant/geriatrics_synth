...
drugs_dat <- study_arm_dat |>
  ...
  select(-matches("\\.x|\\.y")) |>
  mutate(
  # FIXME: fix for 13171 Mahrose 2021 Melatonin/Dex vs Dex 
...
# VARIABLE: drug_recode - includes combination
# VARIABLE: drug_recode_abbr - includes combination abbreviated
...
ket_by_arm <- ket_by_arm |> 
  ...
  select(study, year, refid, study_l, arm_n, drug_recode_abbr, asa_ps_incl, volatile_tab, tiva_tab, regional_tab, sedation_only_tab, age_table, pre_mmse, dose, dose_iv, pre, ind, intr, post, ket_groups)
...
ket_by_arm |> 
  ...
  tab_style(style = cell_text(align = "right"), locations = cells_body(columns = c(arm_n))) |>
  tab_style(style = cell_text(align = "left"), locations = cells_body(columns = c(study_l, drug_recode_abbr, pre:post))) |>
  ...
  tab_style(style = list(cell_text(color = "#A93226")), locations = cells_body(columns = c(arm_n, drug_recode_abbr:post), rows = drug_recode_abbr == "Ket")) |>
  sub_missing(columns = everything(), rows = everything(), missing_text = "") |>
  sub_values(values = c("NS"), replacement = "—") |>
  ...
  tab_footnote(footnote = "Time administered not reported.", locations = cells_body(columns = pre, rows = arm_n == 38 & drug_recode_abbr == "Ket")) |>
...
mel_ram_by_arm <- mel_ram_by_arm |>
  ...
  select(study, year, refid, study_l, arm_n, drug_recode_abbr, asa_ps_incl, volatile_tab, tiva_tab, regional_tab, sedation_only_tab, age_table, pre_mmse, dose, pre, post, mel_ram_groups)
...
mel_ram_by_arm |>
  ...
  tab_spanner(label = "ASA", columns = c(asa_ps_incl)) |>
  tab_spanner(label = "Anesthetic", columns = c(volatile_tab:regional_tab)) |>
  tab_style(style = cell_text(align = "left"), locations = cells_column_labels(columns = c(drug_recode_abbr:post))) |>
  tab_style(style = cell_text(align = "right"), locations = cells_body(columns = c(arm_n))) |>
  tab_style(style = cell_text(align = "left"), locations = cells_body(columns = c(study_l, drug_recode_abbr, dose, pre, post))) |>
  tab_style(style = list(cell_text(color = "#A93226")), locations = cells_body(columns = c(arm_n:post), rows = drug_recode_abbr %in% c("Mel", "Ram"))) |>
  tab_style(style = cell_text(size = px(9)), locations = cells_column_labels(columns = c(volatile_tab:regional_tab))) |>
  tab_style(style = cell_text(size = px(10)), locations = cells_column_labels(columns = c(pre, post))) |>
  sub_missing(columns = everything(), rows = everything(), missing_text = "") |>
  sub_values(values = c("NS"), replacement = "—") |>
  ...
  tab_footnote("GI: gastrointestinal; Abd: abdominal (includes hepatic); Ortho: orthopedic; Various: more that one procedure category; PS: physical status; Vol: volatile; TIVA: total intravenous anesthesia; Reg: regional; MMSE: Mini-Mental State Exam; Pre: preoperative; Post: postoperative; Mel: melatonin; Ram: ramelteon; Oxaz: oxazepam; Dex: dexmedetomidine; Mid: midazolam; Plac; placebo.") |> 
  tab_footnote(md("Mean <u>Med</u> (SD)[Range]{IQR}."), locations = cells_column_labels(columns = c(age_table, pre_mmse))) |> 
  tab_footnote(md("ASA Physical Status of patients included (proportions can be found here[link to table add])."), locations = cells_column_labels(columns = c(asa_ps_incl))) 
...
delirium_meta_dat <- dichot_dat |>
  ...
  select(refid, refid_c, arm_id, year, study, design_f_lab, country, surg_f, postop_only, postop_any, arm, delitotal_n, arm_n, delitotal_perc, delirium_inc_prop) |>
...
#### <font size = 4> `r figure_ref()` Delirium incidence by surgical category across all treatment arms in randomized clinical trials (nb, the "Any" and "Plac/None" box plots are not mutually exclusive) . </font>
...
```{r nma_dat, fig.width = 14, fig.height = 8.5, fig.align = "left", warning = TRUE}
...
dex_meta_dat <- pairwise_dat |>
  ...
  mutate(
    ...
    delirium_inc_prop = ifelse(delirium_inc_prop == TRUE, "Incidence Proportion", "Maximum Daily"),
    study = ifelse(china_vs_other == "China", paste0(study, "*"), paste0(study, " ")),
    rob = case_when(
      Overall == "+" ~ "Low",
...
<foot_mg> Dex: dexmedetomidine; Plac: placebo; RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- . <a id="figure-4"></a><br/> `r paste0("Harbord test for small study effects P = ", formatC(temp_metabias$pval, digits = 4, format = "f"), ".")`<br/> `r paste0("Delirium a designated primary outcome in ", prim_sec_out_fun(prim_delirium, dex_plac_refid), "; a secondary outcome in ", prim_sec_out_fun(sec_delirium, dex_plac_refid), ".")`<br/> \*Trials conducted in China.</foot_mg><br/>

<font size = 4> `r figure_ref()`Dexmedetomidine compared with placebo or no intervention --- trials from China vs. other countries. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention **small study effects**. </font>
...
<foot_mg> Estimates (odds ratio and 95% CI) from random effects models --- unadjusted `r paste0(digit2(exp(temp_limit_meta$TE.random)), " (", digit2(exp(temp_limit_meta$lower.random)), "–", digit2(exp(temp_limit_meta$upper.random)), ")")`; adjusted for small study effects: `r paste0(digit2(exp(temp_limit_meta$TE.adjust)), " (", digit2(exp(temp_limit_meta$lower.adjust)), "–", digit2(exp(temp_limit_meta$upper.adjust)), ")")`.<br/> \*Trials conducted in China.</font>
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention **small study effects --- trials conducted in China**. </font>
...
<foot_mg> Estimates (odds ratio and 95% CI) from random effects models --- unadjusted `r paste0(digit2(exp(temp_limit_meta$TE.random)), " (", digit2(exp(temp_limit_meta$lower.random)), "–", digit2(exp(temp_limit_meta$upper.random)), ")")`; adjusted for small study effects: `r paste0(digit2(exp(temp_limit_china_meta$TE.adjust)), " (", digit2(exp(temp_limit_china_meta$lower.adjust)), "–", digit2(exp(temp_limit_china_meta$upper.adjust)), ")")`.<br/> `r paste0("Harbord test for small study effects P = ", formatC(temp_china_metabias$pval, digits = 4, format = "f"), ".")`<br/></font>
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention **potential small study effects --- trials conducted outside of China**. </font>
...
<foot_mg> Estimates (odds ratio and 95% CI) from random effects model. Too few studies for tests of small study effects.</font>
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention --- **postoperative administration only** or other times. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention --- **any postoperative administration** or other times. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention **by surgical category**. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` <a id="figure-china"></a> Dexmedetomidine compared with placebo or no intervention according to reporting incidence proportion. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention according to risk of bias appraisal. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Summary of subgroup results. </font>
...
subgroup_dat <- readxl::read_excel(
  "data/dex_subgroups.xlsx",
  sheet = "data",
  range = "A1:H21"
) |> 
  mutate(
    Subgroup = ifelse(!is.na(Studies), paste0("   ", Subgroup), Subgroup),
...
![](docs/assets/subgroup_summary_crop.png){#subgroup_summary fig-align="center" width="750"}
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> Exploratory component network meta-analysis examining timing of dexmedetomidine administration. </font>
...
<foot_mg> Excludes Lee 2018b pregabalin arms (pregabalin alone and with dexmedetomidine). </foot_mg>
:::
...
<font size = 4> `r figure_ref()` Melatonin and ramelteon compared with placebo or no intervention. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention small study effects --- funnel plot (delayed neurocognitive recovery). </font>
...
<font size = 4> `r table_ref()` Neurocognitive disorder and ascertainment reported in dexmedetomidine and melatonin trials. </font>
...
<font size = 4> `r table_ref()` Bradycardia in dexmedetomidine studies (randomized clinical trials unless noted). </font>
...
<font size = 4> `r table_ref()` Hypotension in dexmedetomidine studies (randomized clinical trials unless noted). </font>
...
<font size = 4> `r table_ref()` Other reported complications in dexmedetomidine studies --- cardiac, MI, stroke, pulmonary, pneumonia, and renal (randomized clinical trials unless noted). </font>
...
<font size = 4> `r table_ref()` Report complications in ramelteon studies --- cardiac, MI, stroke, pulmonary, pneumonia, and renal (all before-after designs). </font>
...
<font size = 4> `r figure_ref()` Risk ratio for bradycardia (dexmedetomidine versus placebo or no interventions; randomized clinical trials). </font>
...
forest(adverse_meta,
  ...
  just.addcols.right = "left",
  rightcols = c("effect", "ci", "brady_defn", "D1", "D2", "D3", "D4", "D5", "Overall"),
  rightlabs = c("RR", "(95% CI)", "Bradycardia \nCriteria", "D1", "D2", "D3", "D4", "D5", "All"),
...
<foot_mg> RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- ; NR: not reported.<br/> `r paste0("Harbord test for small study effects P = ", digit2(temp_metabias$pval), ".")`<br/> Similar arms combined in Li 2021b and Zhao 2020. </foot_mg>

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention small study effects and bradycardia --- funnel plot and limit meta-analysis. </font>
...
<font size = 4> `r figure_ref()` Risk ratio for hypotension (dexmedetomidine versus placebo or no interventions; randomized clinical trials). </font>

```{r complication_hypo_meta, fig.width = 14.5, fig.height = 6.5, fig.align = "left"}
#| warning: false
# RESUME: hypotension
...
forest(adverse_meta,
  ...
  # rightlabs = c("RD ", "(95% CI)   "),
  just.addcols.right = "left",
  rightcols = c("effect", "ci", "detail_cardiac", "D1", "D2", "D3", "D4", "D5", "Overall"),
  ...
  xlab = "Favors: Dexmedetomidine                     Plac/None",
  addrows.below.overall = 4,
  text.addline1 = "NS: not specified."
)
```

<foot_mg> RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- ; NS: not specified.<br/> `r paste0("Harbord test for small study effects P = ", digit2(temp_metabias$pval), ".")`<br/> Similar arms combined in Li 2021b and Zhao 2020. </foot_mg>

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention small study effects and hypotension --- funnel plot. </font>
...
<font size = 4> `r figure_ref()` Risk ratios for other complications --- cardiac, stroke, pulmonary, and renal (dexmedetomidine versus placebo or no interventions; randomized clinical trials). </font>

```{r complication_meta,  fig.width = 14, fig.height = 16.5, fig.align = "left"}
adverse_meta <- metabin(
  n_Dex, arm_n_Dex, n_Plac, arm_n_Plac,
  ...
  subset = complication != "Bradycardia" & !grepl("\\*", study),
  allstudies = FALSE
)  
...
<foot_mg> RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- .<br/> Similar arms combined in Li 2021b and Zhao 2020. </foot_mg>
...
<foot_mg> MD: mean difference; Dex: Dexmedetomidine; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- .<br/> When the mean or standard deviation were not reported, they were imputed from from the median, interquartile range, and/or range. </foot_mg>
...
<font size = 4> `r figure_ref()` Risk ratio for hospital mortality -- dexmedetomidine compared with placebo or no intervention. </font>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Mortality for dexmedetomidine compared with placebo or no intervention small study effects --- funnel plot. </font>
...
<font size = 4> `r figure_ref()` Risk ratio for hospital mortality -- melatonin or ramelteon compared with placebo or no intervention. </font>
