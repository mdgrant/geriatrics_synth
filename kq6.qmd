...
Updated dexmedetomidine and melatonin/ramelteon results with 3 additional studies reporting daily, but not overall, incidence proportion (cumulative incidence).
:::
...
study_char_dat <- data_kq(study_char_dat, kq6_refid) |> filter(!refid %in% 2598) 
study_arm_dat <- data_kq(study_arm_dat, kq6_refid) |> filter(!refid %in% 2598)
contin_dat <- data_kq(contin_dat, kq6_refid) |> filter(!refid %in% 2598)
dichot_dat <- data_kq(dichot_dat, kq6_refid) |> filter(!refid %in% 2598)
likert_dat <- data_kq(likert_dat, kq6_refid) |> filter(!refid %in% 2598)
...
drugs_dat <- study_arm_dat |>
  ...
  mutate(
    ...
    drug_recode = fct(drug_recode,
      levels = c("Dexmedetomidine", "Dexmedetomidine/Pregabalin", "Dexmedetomidine/Ulinastatin", "Ketamine", "Melatonin", "Melatonin/Dexmedetomidine","Ramelteon", "Ulinastatin", "Haloperidol", "Midazolam", "Midazolam/propofol", "Midazolam/sevoflurane", "Morphine", "Pregabalin", "Propofol", "Rhythim", "Sufentanil", "Placebo", "None")
    ),
...
```{r non_missings_delirium}
#| eval: false
# NOTE: !verified non missing delirium incl effects; all have NS
temp_deli <- dichot_dat |>
  select(refid, study, starts_with("deli")) |>
  filter(if_any(starts_with("deli"), ~ !is.na(.x)))
...
# get max of daily
# dichot_dat |>
#   select(refid, arm_id, study, arm_n, delirium_n1, delirium_n2, delirium_n3, delilast_n, delitotal_n) |>
#   mutate(
#     delirium_day_max = pmax(delirium_n1, delirium_n2, delirium_n3, delilast_n, na.rm = TRUE)
#   ) |>
#   filter(is.na(delitotal_n) & !is.na(delirium_day_max))
...
delirium_meta_dat |>
  ...
  mutate(
        surg_f = fct_rev(factor(surg_f, levels = c("Ortho", "Cardiac", "GI/Abd", "Thoracic", "Oralmax", "Spine", "Various", "Any")))
  ) |> 
...
delirium_total_gt_fun("Dex") |>
  tab_footnote("RR: risk ratio; MMSE: Mini-Mental State Examination; Dex: Dexmedetomidine; Mid: Midazolam; Prop: Propofol; MS: Morphine; Preg: Pregabalin; Prop: Propofol; Rhyth: Rhythim; Plac: placebo; CAM: Confusion Assessment Method; DSM: Diagnostic and Statistical Manual of Mental Disorders; ICDSC: Intensive Care Delirium Screening Checklist; Psych: psychiatrist interview; NS; not specified.") |>
  # tab_footnote(md("Mean <u>Med</u> (SD) [Range] {IQR}."), locations = cells_column_labels(columns = c(age_table, pre_mmse))) |>
...
delirium_total_gt_ket |>
  tab_footnote("RR: risk ratio; MMSE: Mini-Mental State Examination; Ket: ketamine; Plac: placebo; Hal: haloperidol; ICDSC: Intensive Care Delirium Screening Checklist; Psych: psychiatrist interview; NS; not specified.") |>
...
<foot_mg>  Dex: Dexmedetomidine; Ket: Ketamine; Mel/Ram: Melatonin or ramelteon; Mid: Midazolam; MS: Morphine sulfate; Plac/None: Placebo or no intervention; Prop: Propofol; Rhyth: Rhythim. </foot_mg> 
...
```{r nma_dat, fig.width = 14, fig.height = 8, fig.align = "left"}
...
dex_meta_dat <- pairwise_dat |> 
  ...
  select(study, year, event2, n2, event1, n1, D1:Overall, china_vs_other, postop_only) |> 
  mutate(postop_only = ifelse(!is.na(postop_only), "Only postop", "Includes other times"))

# dex_meta_dat |> 
#   summarize(tot = sum(n2) + sum(n1))
...
forest(dex_plac_meta,
  ...
  text.addline2 = "He 2018, Huyan 2019, Gao 2021, and Lu 2021 incidence proportion based on the maximum daily reported."
)
```
<foot_mg>  Dex: dexmedetomidine; Plac: placebo; RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias; +: low risk of bias; ?: some concerns; - high risk of bias. </foot_mg> <a id="figure-4"></a>
...
dex_plac_meta_china <- metabin(event2, n2, event1, n1,
  ...
  allstudies = TRUE,
  subgroup = china_vs_other
  # subgroup = postop_only
  )

temp_metareg <- update(dex_plac_meta_china, tau.common = TRUE, common = FALSE, method.random.ci = "HK")
...
forest(dex_plac_meta_china,
  ...
  addrows.below.overall = 4,
  text.addline1 = paste0("Harbord test for small study effects P = ", digit2(temp_metabias$pval)),
  text.addline2 = paste0("Meta-regression (China vs. other) I² = ", digit0(temp_metareg$I2), "%"))

```

<foot_mg>  Dex: dexmedetomidine; Plac: placebo; RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias; +: low risk of bias; ?: some concerns; - high risk of bias. </foot_mg> 
:::

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention --- postoperative administration only vs. other times. </font>

```{r nma_dat_postop, fig.width = 14, fig.height = 10, fig.align = "left"}
dex_plac_meta_postop <- metabin(event2, n2, event1, n1,
  data = dex_meta_dat,
  studlab = study,
  sm = "RR",
  method = "MH",
...
```{r dex_plac_meta_funnel_limit, out.width = "85%", out.height = "100%", fig.align = "left"}
par(mar = c(4, 4, 2, 1))
...
funnel(temp_limit_meta, ylim = c(0.8, 0.0), xlim = c(0.1, 5), pch = 20, contour = c(0.9, 0.95, 0.99), col.contour = limit_colors, studlab = TRUE, fs.smlab = 7, ff.smlab = "italic")
...
```{r nma_mel_ram_dat, fig.width = 14, fig.height = 6.5, fig.align = "left"}
# ```{r nma_dat, out.width = 50%, out.height = 100%}
# risk of bias
...
```{r nmaForest, fig.width = 6, fig.height = 3.3, fig.align = "center"}
#| eval: true
...
<foot_mg> Dex: Dexmedetomidine; Ket: Ketamine; Mel/Ram: Melatonin or Ramelteon; Mid: Midazolam; MS: Morphine; Preg: Pregabalin; Prop: Propofol; Rhyth: Rhythim; Plac/None: Placebo or no comparator. </foot_mg>
...
as_tibble(nma_melram_league$random) |>
  ...
  tab_footnote("Dex: Dexmedetomidine; Ket: Ketamine; Mel/Ram: Melatonin or Ramelteon; Mid: Midazolam; MS: Morphine; Preg: Pregabalin; Prop: Propofol; Rhyth: Rhythim; Plac/None: Placebo or no comparator.")
...
<foot_mg> Dex: Dexmedetomidine; Ket: Ketamine; Mel: Melatonin; Ram: Ramelteon; Mid: Midazolam; MS: Morphine; Preg: Pregabalin; Prop: Propofol; Rhyth: Rhythim; Plac/None: Placebo or no comparator. </foot_mg> 
:::
...
<font size = 4> `r table_ref()` Reported in-hospital, 30-, 90-, and 1-year mortality by comparator and surgical category.</font>

```{r non_missings_mortality}
#| eval: false
# NOTE: !verified non missing mortality incl effects; all have ns
temp_mort <- dichot_dat |>
  select(refid, study, starts_with("mort")) |>
  filter(if_any(starts_with("mort"), ~ !is.na(.x)))

skim(temp_mort)
writexl::write_xlsx(temp_mort, "/Users/mgrant/Desktop/temp_mort.xlsx")
```

```{r mortality_data}
# CODE: mortality_dat create data including all reported, hosp, 30, 60, 90, 1-ear
mortality_total_tab_fun <- function(refids, mort_est, mort_period) {
  # referent values to calculate rr or rd
  mortality_rr_ref <- dichot_dat |>
    mutate(
      mort_period = mort_period,
      mort_n = {{ mort_est }},
      ref_mort_n = mort_n,
      ref_arm_n = arm_n
    ) |>
    select(refid, refid_c, arm_id, year, study, study_l, design_f_lab, mort_n, arm_n, ref_mort_n, ref_arm_n, mort_period) |>
    filter(!is.na(mort_n)) |>
    filter(refid %in% refids) |>
    arrange(refid_c, arm_id) |>
    # rename(ref_mort_n = mort_n, ref_arm_n = arm_n) |>
    group_by(refid_c) |>
    mutate(
...
mortality_dat <- bind_rows(
  mortality_total_tab_fun(kq6_refid, morthosp_n, "Hospital"),
  mortality_total_tab_fun(kq6_refid, mort30_n, "30d"),
  mortality_total_tab_fun(kq6_refid, mort90_n, "90d"),
  mortality_total_tab_fun(kq6_refid, mort1_n, "1yr")) |>
  ...
  mutate(mort_percent = mort_n/arm_n,
         mort_period = factor(mort_period, levels = c("Hospital", "30d", "90d", "1yr"))) |>
  relocate(mort_percent, .after = arm_n) |> 
  relocate(arm_n, drug_recode_abbr, asa_ps_incl, age_table, .after = study_l)
```
...
mortality_dat |>
  ...
  gt(id = "one") |> 
  cols_hide(c(refid, refid_c, arm_id, year, study, design_f_lab, mort_percent, ref_mort_n, ref_arm_n)) |>
  row_group_order(groups = c("Hospital", "30d", "90d", "1yr")) |> 
  cols_label(
    study_l = "Study",
    arm_n = "N",
    drug_recode = "Drug",
  ...
  tab_style(style = list(cell_text(color = "#0E6655")), locations = cells_body(columns = c(arm_n:mort_n_percent), rows = str_detect(drug_recode, "Mel|Ram"))) |> 
  tab_style(style = list(cell_text(color = "#F39C12")), locations = cells_body(columns = c(arm_n:mort_n_percent), rows = str_detect(drug_recode, "Ulin|MS|Prop"))) |>
  tab_style(style = list(cell_text(color = "#A93226")), locations = cells_body(columns = c(arm_n:mort_n_percent), rows = str_detect(drug_recode, "Dex"))) |> 
  sub_missing(columns = everything(), rows = everything(), missing_text = "") |> 
  cols_width(
    study_l ~ px(140),
    arm_n ~ px(45),
  ...
  # opt_css(css = "#one .gt_column_spanner {border-bottom-style: hidden;}") |>
  tab_footnote("ASA PS: American Society of Anesthesiologists Physical Status; RD: risk difference; GI: gastrointestinal; Abd: abdominal (includes hepatic); Ortho: orthopedic; Various: more that one procedure category;  Dex: Dexmedetomidine; Ulin: Ulinastatin; Hal: Haloperidol; Mid: Midazolam; Mid/Prop: Midazolam/propofol; Mid/Seve: Midazolam/sevoflurane; Preg: Pregabalin; Prop: Propofol; Rhyth: Rhythim; Plac; placebo;") |> 
