...
# CODE: drugs_dat <- study_arm_dat include drug arm data; adds kq5/6 arms
...
```{r drug_tables}
#| eval = FALSE
# CODE: drug and drug_recode can save to desktop; working only
...
```{r dex_delirium_descriptive}
temp <- study_arm_dat |> 
  select(refid, study, matches("^age"))
...
temp_2 <- likert_dat |> 
  select(refid, study, matches("mmse")) |> 
  filter(if_any(starts_with("mmse"), ~ !is.na(.)))

temp_contin <- contin_dat |>
  select(refid, study, matches("^deli")) |>
  filter(if_any(starts_with("deli"), ~ !is.na(.)))g

mmse_refids <- temp_contin$refid  
mmse_delirium_refids <- temp_contin$refid  

mmse_refids %in% kq6_refid
