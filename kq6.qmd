...
<!-- ## Used for communicating recent updates -->
<!-- | Date       | Modification                                                                                                                                                                    | -->
<!-- |:-----------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| -->
<!-- |            | 
<!-- ::: -->
...
dichot_dat <- data_kq(dichot_dat, kq6_refid) |>
  mutate(
    delirium_inc_prop = !is.na(delitotal_n), # reported incidence proportion <lgl>
    delirium_day_max = pmax(
      delirium_n1,
      delirium_n2,
      delirium_n3,
      delilast_n,
...
```{r drugs_by_arm}
#| label: drugs_by_arm
# recode drugs/comparators so that usable; "other" category initially recode to comparator & below identified from text field
...
kq5_drug_by_arm <- read_csv(
  "data/kq56_drug_by_arm.csv",
  col_types = c("n", "n", "c", "c")
) |>
...
## convert to factors w/clean labels for analysis ----- (2024-04-14 08:45) @----
# only drug_study dex ket mel ram
drugs_dat <- drugs_dat_prelim |>
  ...
  mutate(
    ...
    drug_study = fct(
      drug_study,
      levels = c("Dexmedetomidine", "Ketamine", "Melatonin", "Ramelteon")
    ),
    drug_recode = case_when(
      str_detect(other_spec, "Midazolam and propofol") ~ "Midazolam/propofol",
      str_detect(other_spec, "Midazolam and sevoflurane") ~
        "Midazolam/sevoflurane",
    ...
    drug_recode = ifelse(
      refid == 13406 & arm_id == 3,
      "Dexmedetomidine/Ulinastatin",
      drug_recode
    ),
    drug_recode = ifelse(
      refid == 13393 & arm_id == 1,
      "Dexmedetomidine/Ulinastatin",
      drug_recode
    ),
  ...
  ungroup() |>
  relocate(
    c(drug, drug_f, drug_recode, drug_recode_abbr, drug_study),
    .after = arm_id
  ) |>
...
<caption_mg> Benefits, harms, and strength of evidence (GRADE) for dexmedetomidine versus placebo or no delirium prophylaxis. </caption_mg>
...
<font size = 4> Other complications and strength of evidence (GRADE) for dexmedetomidine versus placebo or no delirium prophylaxis. </font>
...
dichot_freq_fun(dichot_dat |> filter(refid %in% dex_refid), "ADL: activities of daily living; DNCR: [delayed neurocognitive recovery](https://pubmed.ncbi.nlm.nih.gov/30325806/) (neurocognitive disorder < 30 days); POCD: [postoperative neurocognitive disorder](https://pubmed.ncbi.nlm.nih.gov/30325806/) (neurocognitive disorder 30 days to 1 year); QoR: quality of recovery; ")
```
...
See [Appendix](summary_kq6.html) for detailed summary study and patient characteristics including primary outcomes.
...
dex_by_arm |> 
  ...
  tab_style(style = list(cell_text(color = "#104E8B")), locations = cells_body(columns = c(arm_n:timing_all), rows = str_detect(drug_recode_abbr, "Prop|Mid|Preg|Dex/Preg|Ket|Rem"))) |> 
...
<foot_mg>Dex: dexmedetomidine; Ket: ketamine; Mel: melatonin; Ram: ramelteon; Oxaz: oxazepam; MS: morphine sulfate; Prop: propofol; Plac: placebo or no delirium prophylaxis. <br/> The "Any" and "Plac" box plots are not mutually exclusive. <br/> Outliers not designated. </foot_mg>
:::
...
png("assets/kq6_dex_delirium_all.png", width = 12, height = 10, units = "in", res = 300)
dex_meta_forest(dex_plac_meta, text.addline2 = rd_ci_from_meta(dex_plac_meta, pscale = 100, digits = 1))
dev.off() 

trim_image("kq6_dex_delirium_all.png")

# sort by size
# dex_meta_forest(dex_plac_meta, sortvar = dex_plac_meta$data$size)

# for soe/grade summary 6.01 excel file
# rd_ci_from_meta_soe(dex_plac_meta, digits = 1)
# soe_meta_result_rr_or(dex_plac_meta)

```

![](assets/kq6_dex_delirium_all.png){fig.align="left" width="79%"}

<foot_mg> Dex: dexmedetomidine; Plac: placebo; RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/> Risk of bias ratings: low +, some concerns ?, high -- . <a id="figure-4"></a><br/> `r paste0("Harbord test for small study effects P = ", formatC(temp_metabias$pval, digits = 5, format = "f"), ".")`<br/> `r paste0("Delirium a designated primary outcome in ", prim_sec_out_fun(prim_delirium, dex_plac_refid), "; a secondary outcome in ", prim_sec_out_fun(sec_delirium, dex_plac_refid), ".")`<br/> Excluding Turan 2020: RR 0.56 (95% CI, 0.49-0.64; *I*<sup> 2</sup> = 13%, $\tau$<sup>2</sup> = 0.04. <br/>\*Trials conducted in China.</foot_mg>
...
png("assets/kq6_dex_delirium_country.png", width = 11.61, height = 11.5, units = "in", res = 300)
dex_meta_forest_sub(dex_plac_country_meta)
dev.off()

trim_image("kq6_dex_delirium_country.png")
...
![](assets/kq6_dex_delirium_country.png){fig.align="left" width="80%"}
...
png("assets/kq6_dex_plac_meta_postop.png", width = 11.61, height = 12, units = "in", res = 300)
dex_meta_forest_sub(dex_plac_meta_postop)
dev.off()

trim_image("kq6_dex_plac_meta_postop.png")

rm(dex_plac_meta_postop)
```

![](assets/kq6_dex_plac_meta_postop.png){fig.align="left" width="80%"}
...
png("assets/kq6_dex_plac_meta_postop_any.png", width = 11, height = 12, units = "in", res = 300)
dex_meta_forest_sub(dex_plac_meta_postop_any)
dev.off()

trim_image("kq6_dex_plac_meta_postop_any.png")

rm(dex_plac_meta_postop_any)
```
![](assets/kq6_dex_plac_meta_postop_any.png){fig.align="left" width="80%"}
...
png("assets/kq6_dex_plac_meta_ortho.png", width = 12, height = 7.5, units = "in", res = 300)
dex_meta_forest_surg_sub(dex_plac_meta, "Ortho", "HK", "HK")
dev.off()
trim_image("kq6_dex_plac_meta_ortho.png")

png("assets/kq6_dex_plac_meta_various.png", width = 12, height = 7.5, units = "in", res = 300)
dex_meta_forest_surg_sub(dex_plac_meta, "Various", "HK", "HK")
dev.off()
trim_image("kq6_dex_plac_meta_various.png")

png("assets/kq6_dex_plac_meta_cardiac.png", width = 12, height = 7.5, units = "in", res = 300)
dex_meta_forest_surg_sub(dex_plac_meta, "Cardiac", "HK", "HK")
dev.off()
trim_image("kq6_dex_plac_meta_cardiac.png")

png("assets/kq6_dex_plac_meta_thoracic.png", width = 12, height = 7.5, units = "in", res = 300)
dex_meta_forest_surg_sub(dex_plac_meta, "Thoracic", "HK", "HK")
dev.off()
trim_image("kq6_dex_plac_meta_thoracic.png")

png("assets/kq6_dex_plac_meta_gi-abd.png", width = 12, height = 7.5, units = "in", res = 300)
...
png("assets/kq6_dex_rob_meta.png", width = 12, height = 12.65, units = "in", res = 300)
dex_meta_forest_sub(dex_rob_meta)
dev.off()

trim_image("kq6_dex_rob_meta.png")

rm(dex_rob_meta)
```
![](assets/kq6_dex_rob_meta.png){fig.align="left" width="80%"}
...
png("assets/kq6_delirium_inc_pro_meta.png", width = 12, height = 11.5, units = "in", res = 300)
dex_meta_forest_sub(delirium_inc_pro_meta)
dev.off()

trim_image("kq6_delirium_inc_pro_meta.png")

rm(delirium_inc_pro_meta)
```
![](assets/kq6_delirium_inc_pro_meta.png){fig.align="left" width="80%"}
...
png("assets/kq6_dex_plac_base_sub_meta.png", width = 12, height = 14, units = "in", res = 300)
...
![](assets/kq6_dex_plac_base_sub_meta.png){fig.align="left" width="80%"}
:::
...
![](assets/subgroup_summary.png){#subgroup_summary width="95%"}
...
forest(ket_plac_meta,
  ...
  addrows.below.overall = 4,
  addrows.below = 1,
  text.addline2 = rd_ci_from_meta(ket_plac_meta, pscale = 100, digits = 1) 
)
dev.off()

# soe_meta_result_rr_or(ket_plac_meta, "RR")
# rd_ci_from_meta_soe(ket_plac_meta, digits = 1)
```
...
forest(mel_ram_plac_meta,
  ...
  addrows.below.overall = 4,
  text.addline2 = rd_ci_from_meta(mel_ram_plac_meta, pscale = 100)
...
delirium_nma_char$network |> 
  ...
  tab_header(title = glue::glue("{table_ref()}Summary of studies included in the network for delirium incidence.")) |> 
...
delirium_nma_char$comparison |>
  ...
  tab_header(title = glue::glue("{table_ref()}Summary of events by comparisons in the network for delirium incidence.")) |>
...
<caption_mg> `r table_ref()` League table of risk ratios and 95% confidence intervals from network meta-analysis of delirium incidence (random effects model). Comparisons from direct evidence displayed in the upper diagonal; network comparisons shown on the lower diagonal. </caption_mg>
...
<caption_mg> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention (randomized clinical trials, neurocognitive disorder <30 days assessed on postoperative day 5 or later). </caption_mg>
...
png("assets/kq6_dncr_dex_plac_meta.png", width = 12, height = 8, units = "in", res = 300)
forest(dncr_dex_plac_meta,
  ...
  xlab = "Favors: Dexmedetomidine               Placebo/None",
  addrows.below.overall = 4,
  text.addline2 = rd_ci_from_meta(dncr_dex_plac_meta, pscale = 100)
)  
dev.off()

trim_image("kq6_dncr_dex_plac_meta.png")
...
`r paste0("Neurocognitive recovery a designated primary outcome in ", prim_sec_out_fun(prim_pocd, dex_meta_dat$refid), "; a secondary outcome in ", prim_sec_out_fun(sec_pocd, dex_meta_dat$refid), ".")`<br/> \*Trials conducted in China.</foot_mg><br/>

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention according to risk of bias appraisals (randomized clinical trials; neurocognitive disorder <30 days). </caption_mg>
...
png("assets/kq6_dncr_dex_rob_meta.png", width = 12, height = 8, units = "in", res = 300)
forest(rob_meta,
  allstudies = TRUE,
  random = TRUE,
  label.e = "Dex     ",
...
trim_image("kq6_dncr_dex_rob_meta.png")
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention small study effects --- funnel plot (neurocognitive disorder <30 days). </caption_mg>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Summary risk of bias from randomized clinical trials of dexmedetomidine compared with placebo or no intervention (neurocognitive disorder <30 days, weighted). </caption_mg>
...
#| warning: false
#| layout: [[40, 60]]
delirium_nma_dncr_bugsnet_dat <- suppressWarnings(BUGSnet::data.prep(
...
delirium_nma_char$network |> 
  ...
  tab_header(title = glue::glue("{table_ref()}Summary of studies included in the network for neurocognitive disorder <30 days.")) |> 
  fmt_number(columns = c(Value), drop_trailing_zeros = TRUE) |>
  gt_theme_mg()
...
delirium_nma_char$comparison |>
  ...
  tab_header(title = glue::glue("{table_ref()}Summary of events by comparisons in the network for neurocognitive disorder <30 days."), preheader = "Test") |>
  fmt_number(columns = c(n.patients), drop_trailing_zeros = TRUE) |>
  gt_theme_mg()
...
<caption_mg> `r figure_ref()` Network structure of randomized clinical trials and comparators reporting neurocognitive disorder <30 days. </caption_mg>
...
<caption_mg> `r figure_ref()` Network meta-analysis of neurocognitive disorder <30 days including all comparators.</caption_mg>
...
<caption_mg> `r table_ref()` League table of risk ratios and 95% confidence intervals from network meta-analysis of neurocognitive disorder <30 days (random effects model). Comparisons from direct evidence displayed in the upper diagonal; network comparisons shown on the lower diagonal. </caption_mg>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Comparison-adjusted funnel plot for neurocognitive disorder <30 days. </caption_mg>
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` Comparison of direct and indirect evidence in network meta-analysis of neurocognitive disorder <30 days (node splitting forest plot). </caption_mg>
...
<caption_mg> `r table_ref()` Neurocognitive disorder 30 days to 1 year and ascertainment. </caption_mg>
...
ncd_dat |>
  ...
  sub_missing(columns = everything(), missing_text = "") |>
  tab_spanner(label = "Instrument", columns = c(scale_mmse:scale_other_tf), level = 1) |>
  tab_spanner(label = "Neurocognitive Disorder 30 days to 1 yr", columns = c(neurocog_nper:rr_ci), level = 1) |>
  tab_spanner(label = "Preop", columns = c(pre_mmse), level = 1) |>
  cols_width(
    study_l ~ px(165),
    arm_n ~ px(45),
...
<a id="fig_pooled_dex"></a>

### *Pooled*

#### Dexmedetomidine

<caption_mg> `r figure_ref()` Neurocognitive disorder 30 days to 1 year comparing dexmedetomidine with placebo. </caption_mg>  
...
<caption_mg> `r figure_ref()` Neurocognitive disorder 30 days to 1 year comparing ketamine with placebo. </caption_mg>  
...
:::
<foot_mg> Excludes midazolam and propofol arms as neither contributed (ie, not in a closed loops).  </foot_mg> 

:::

## **Physical Function**

<caption_mg> `r table_ref()` Physical functional status following dexmedetomidine and melatonin for delirium prophylaxis. </caption_mg>
...
adverse_meta_dat <- adverse_events_dat |>
  ...
  mutate(
    ...
    complication = case_when(
      ...
      detail_pulmonary %in% c("respiratory depression") ~ "respfail",
      .default = complication
    ),
    complication = factor(complication, levels = c("bradycardia", "other arrhythmia", "myoinfarct", "cardarr", "kidneyinj", "stroke", "pulm", "pneum", "pulcongest", "pe", "respfail", "intub"), labels = c("Bradycardia", "Other arrhythmia", "Myocardial Infarction", "Cardiac Arrest", "Acute Kidney Injury", "Stroke", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "Pulmonary Embolism", "Respiratory Failure", "Reintubation"))
  ) |>
  select(refid, year, study, arm_id, arm, arm_n, n, complication, study_compl, detail_cardiac, detail_pulmonary)
...
<caption_mg> `r table_ref()` Other complications reported in dexmedetomidine studies --- cardiac, myocardial infarction, stroke, renal, and respiratory (randomized clinical trials unless noted). </caption_mg>
...
adverse_events_dat |> 
  ...
  # filter(!refid %in% mel_ram_refid) |> tabyl(complication) |> arrange(n)
  filter(complication != "Bradycardia") |> 
  filter(any(arm %in% "Dex"), .by = study) |> # filter(!any(arm %in% "Plac"), .by = study) |> -> temp  # NOTE: Cheng 2016, Djainai 2016, Wang 2020a, Zhao 2020 no placebo
  mutate(
    complication = factor(complication,
      levels = c("Myocardial Infarction", "Cardiac Arrest", "Other arrhythmia", "Stroke", "Acute Kidney Injury", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "Pulmonary Embolism", "Respiratory Failure"),
      labels = c("Myocardial Infarction", "Cardiac Arrest", "Other Arrhythmia", "Stroke", "Acute Kidney Injury", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "Pulmonary Embolism", "Respiratory Depression/Failure")
    ),
  ...
  gt(id = "one") |>
  row_group_order(groups = c("Myocardial Infarction", "Cardiac Arrest", "Other Arrhythmia", "Stroke", "Acute Kidney Injury", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "Pulmonary Embolism", "Respiratory Depression/Failure")) |>
  ...
  tab_footnote("RD: risk difference.") |>
  ...
  tab_footnote("Includes unspecified, composite, respiratory depression, pleural effusion.", locations = cells_row_groups(groups = "Pulmonary Congestion")) |> 
  tab_footnote("Pulmonary infection, pneumothorax, or pleural effusion needing intervention.", locations = cells_body(columns = study_l, rows = age_table == "67.5 (5.3)" )) |> 
  tab_footnote("Unspecified", locations = cells_body(columns = study_l, rows = age_table == "71.4 (4.5)" & arm == "Plac"))

```

####   *Pooled*

<caption_mg> `r figure_ref()` **Risk ratios** for other complications --- cardiac arrest, myocardial infarction, other arrhythmia (excludes bradycardia), stroke, pulmonary, and renal (dexmedetomidine versus placebo or no interventions). </caption_mg>

```{r complication_rr_meta}
#| label: complication_rr_meta
#| echo: false
#| include: false

adverse_other_dat <- adverse_dex_meta_wide_dat |>
  filter(!complication %in% c("Bradycardia", "Hypotension", "Other arrhythmia")) |> # 3 studies arrhythmia other than bradycardia (a fib, tachycardia, unspecified)
  mutate(
    complication = ifelse(complication == "Respiratory Failure", "Respiratory Depression/Failure", complication),
    complication = ordered(complication, c("Myocardial Infarction", "Cardiac Arrest", "Stroke", "Acute Kidney Injury", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "Pulmonary Embolism", "Respiratory Depression/Failure")),
    study = ifelse(study == "Cheng 2016", "Cheng 2016*", study)
  )

adverse_other_dat$complication
...
# set prediction interval for MI [1] and pneumonia [6] and Respiratory Depression/Failure to missing; implausibly large; matrix(adverse_meta$lower.predict.w)
adverse_meta$lower.predict.w[1] <- NA
adverse_meta$upper.predict.w[1] <- NA
adverse_meta$lower.predict.w[6] <- NA
adverse_meta$upper.predict.w[6] <- NA
adverse_meta$lower.predict.w[9] <- NA
adverse_meta$upper.predict.w[9] <- NA

png("assets/kq6_adverse_dex_other_rr_meta.png", width = 10, height = 18, units = "in", res = 300)
forest(adverse_meta,
  allstudies = TRUE,
  random = TRUE,
  label.e = "Dex",
  ...
  text.addline1 = "Prediction intervals not shown when data insufficient to estimate a valid result.",
  text.addline2 = "* Retrospective cohort."
)
dev.off()
...
```
![](assets/kq6_adverse_dex_other_rr_meta.png){fig.align="left" width="60%"}

<foot_mg> Similar arms combined for Zhao 2020. </foot_mg>

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <caption_mg> `r figure_ref()` **Risk differences** for other complications --- cardiac arrest, myocardial infarction, other arrhythmia (excludes bradycardia), stroke, pulmonary, and acute kidney injury (dexmedetomidine versus placebo or no interventions). </caption_mg>
...
adverse_meta <- metabin(
  ...
  hakn = FALSE,
  subgroup = complication,
  subset = !complication %in%  c("Bradycardia", "Hypotension"),
  allstudies = FALSE
)  

# set prediction interval for pneumonia and pulmonary to missing; implausible large
adverse_meta$lower.predict.w[6] <- NA
adverse_meta$upper.predict.w[6] <- NA
adverse_meta$lower.predict.w[9] <- NA
adverse_meta$upper.predict.w[9] <- NA

png("assets/kq6_adverse_dex_other_rd_meta.png", width = 10, height = 18, units = "in", res = 300)
forest(adverse_meta,
  allstudies = TRUE,
  random = TRUE,
  label.e = "Dex",
...
```
![](assets/kq6_adverse_dex_other_rd_meta.png){fig.align="left" width="70%"}
...
adverse_events_dat |>
  ...
  group_by(complication) |>
  gt(id = "one") |>
  row_group_order(groups = c("Bradycardia", "Hypotension", "Acute Kidney Injury")) |>
  cols_hide(c(refid, year, study, arm_id, n, study, study_compl, design_f_lab)) |>
  cols_label(
    study_l          = "Study",
    arm_n            = " N",
...
<caption_mg> `r table_ref()` Complications reported in ramelteon studies --- cardiac, myocardial infarction, stroke, pulmonary, pneumonia, and renal (all before-after designs). </caption_mg>
...
forest(los_dex_meta,
  ...
  addrows.below.overall = 4,
  text.addline1 = "With Hartung-Knapp adjustment overall 95% CI -1.5 to -0.08"
)
dev.off()
...
<foot_mg> MD: mean difference; Dex: Dexmedetomidine.<br/> When the mean or standard deviation were not reported, they were imputed from from the median, interquartile range, and/or range. Hartung-Knapp adjustment not applied owing to interest in subgroups.</foot_mg>
...
<foot_mg> Dex: dexmedetomidine; Plac: placebo.<br/><foot_mg> \*Trials conducted in China with RR 0.69 (95% CI, 0.41–1.15); others RR 0.59 (95% CI, 0.10–3.37). </foot_mg>
