...
```{r delirium_nma_plot, warning = FALSE, fig.dim = c(6, 5), out.width = "40%", fig.align = "center"}
...
# proportion primary or secondary outcome
prim_delirium_temp <- study_char_dat |> 
  filter(refid %in% dex_plac_refid) |> 
  mutate(percent = mean(!is.na(prim_delirium))) |> 
  select(percent) |> 
  distinct() |> 
  pull()

sec_delirium_temp <- study_char_dat |> 
  filter(refid %in% dex_plac_refid) |> 
  mutate(percent = mean(!is.na(sec_delirium))) |> 
  select(percent) |> 
  distinct() |> 
  pull()
...
forest(dex_plac_meta,
  ...
  text.addline2 = paste0("Delirium a designated primary outcome in ", digit1(prim_delirium_temp * 100), "%; a secondary outcome in ", digit1(sec_delirium_temp * 100), "%.")
)

rm(prim_delirium_temp, sec_delirium_temp)

```

<foot_mg> Dex: dexmedetomidine; Plac: placebo; RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/>
Risk of bias ratings: + low, ? some concerns, - high. </foot_mg> <a id="figure-4"></a>
<br/>

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention according to reporting incidence proportion (randomized clinical trials). </font>
...
<foot_mg> Dex: dexmedetomidine; Plac: placebo; RR: risk ratio; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/>
Risk of bias ratings: + low, ? some concerns, - high. </foot_mg>
:::
...
<foot_mg> D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/>
Risk of bias ratings: + low, ? some concerns, - high. </foot_mg>
<br/>

#### Melatonin/Ramelteon

<font size = 4> `r figure_ref()` Melatonin and ramelteon compared with placebo (randomized clinical trials). </font>
...
```
<a id="figure-11"></a>
<foot_mg> Mel: melatonin; Ram: ramelteon; D1: bias arising from the randomization process; D2: bias due to deviations from intended interventions; D3: bias due to missing outcome data; D4: bias in measurement of the outcome; D5: bias in selection of the reported result: All: overall risk of bias.<br/>
Risk of bias ratings: + low, ? some concerns, - high.</foot_mg>
<br/>
...
pairwise_dat <- pairwise(
  ...
  left_join(robinsi_dat |> select(-study), by = "refid") |> 
  select(study, year, event2, n2, event1, n1, D1:Overall, treat2, treat1) |> 
...
<foot_mg> D1: Bias due to confounding; D2: Bias in selection of participants into the study; D3: Bias in classification of interventions; D4: Bias due to deviations from intended interventions; D5: Bias due to missing data; D6: Bias in measurement of outcomes; D7: Bias in selection of reported results; All: overall risk of bias.<br/>
Risk of bias ratings: ++ low, + moderate, - serious, - - critical; NI: no information; NA: not applicable.</foot_mg>
:::
...
missing_non_n_fun <- function(var_prefix) {
  dichot_dat |>
    filter(if_any(!!paste0(var_prefix, "_per"):!!paste0(var_prefix, "_95high"), ~ !is.na(.x)) & is.na(!!paste0(var_prefix, "_n")))
}
variables_temp <- c("cardarr", "intub", "airleak", "gastro", "medical", "pulcongest", "respfail", "pe", "ssi", "thromb", "kidneyinj", "myoinfarct", "othcomp", "pneum", "infect", "cardiac", "stroke", "pulm", "cardiac")
map(variables_temp, missing_non_n_fun)
...
mortality_dat <- dichot_dat |>
  ...
  select(refid, refid_c, arm_id, year, study, study_l, design_f_lab, arm_n,  matches("_n|per") & starts_with("mort")) |> 
  pivot_longer(
...
<font size = 4> `r table_ref()` Reported in-hospital, 30-, 90-, and 1-year mortality in dexmedetomidine randomized clinical trials by comparator and surgical category.</font>
...
```{r mortality_dex_meta}
#| eval: true
mortality_dex_meta_dat <- mortality_dat |>
  filter(refid %in% dex_mort_refid, mort_period == "Hospital") |>
  rename(arm = drug_recode_abbr) |> 
  filter(!str_detect(arm, "Ulin")) |> 
  select(refid, study, arm, mort_n, arm_n)

trts_mort_dex <- levels(fct_drop(mortality_dex_meta_dat$arm))
...
pairwise_mort_dex_dat <- pairwise(
  ...
  studlab = study,
  allstudies = TRUE,
  data = mortality_dex_meta_dat,
  sm = "RD",
  keep.all.comparisons = TRUE
)

netmeta_mort_dex_nma <- netmeta(
...
forest(netmeta_mort_dex_nma, pscale = 100)

```

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
### <font size = 4> `r figure_ref()` Summary risk of bias from dexmedetomidine randomized clinical trials reporting mortality. </font>
...
<font size = 4> `r table_ref()` Reported in-hospital, 30-, 90-, and 1-year mortality in melatonin and ramelteon studies by comparator and surgical category.</font>
...
```{r dex_mort_meta, fig.width = 14, fig.height = 8, fig.align = "left"}
# ```{r nma_dat, out.width = 50%, out.height = 100%}
# risk of bias
rob_meta_dat <- rob2_reconciled_dat |> 
  select(-Study) |> 
...
dex_mort_meta_dat <- pairwise_mort_dex_dat |> 
  ...
  select(study, year, event2, n2, event1, n1, D1:Overall)
