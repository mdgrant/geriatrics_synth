...
delirium_meta_dat |>
  ...
  scale_colour_manual(values = c("#e41a1c", "#8B0000", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00", "#D4AC0D", "#170a8a")) +
...
dex_plac_meta_surgs <- update(dex_plac_meta, method.random.ci = "classic", subgroup = surg_f)

png("assets/kq6_dex_plac_meta_surgs.png", width = 10.07, height = 15.85, units = "in", res = 300)
dex_meta_forest_sub(dex_plac_meta_surgs)
dev.off()

rm(dex_plac_meta_surgs)
```
![](assets/kq6_dex_plac_meta_surgs.png){fig.align="left" width="70%"}
...
<foot_mg> Similar plot for odds ratios obtained from a Bayesian meta-regression (fitted using Stan and multinma).</foot_mg>  
...
<caption_mg> `r figure_ref()`  Postoperative neurocognitive disorder comparing dexmedetomidine with placebo. </caption_mg>  
...
<caption_mg> `r figure_ref()`  Postoperative neurocognitive disorder comparing ketamine with placebo. </caption_mg>  
...
<caption_mg> `r table_ref()` Other reported complications in dexmedetomidine studies --- cardiac, MI, stroke, renal, and respiratory (randomized clinical trials unless noted). </caption_mg>
...
```{r}
# RESUME: 2024-06-04 14:47 
```
Add Remerand 2009, Ma 2023
...
adverse_dex_meta_wide_dat <- adverse_dex_meta_dat |>
  # mutate(study = ifelse(study %in% c("Li 2020a"), paste0(study, "*"), study)) |>
  filter(!study %in% c("Li 2020a")) |> # not rcts
