...
kq6_refid <- kq6_refid[!kq6_refid %in% c(13734, 5269)] # Fan 2017b rhythim, Gao 2021 duplicate 
...
study_char_dat |>
  ...
  tab_footnote("Studies with multiple publications counted only once (applied to 2 trials with 2 publications).")
```
...
delirium_total_gt_fun("Dex") |>
  tab_footnote("RR: risk ratio; MMSE: Mini-Mental State Examination; Dex: Dexmedetomidine; Mid: Midazolam; Prop: Propofol; MS: Morphine; Preg: Pregabalin; Plac: placebo; DSM: Diagnostic and Statistical Manual of Mental Disorders; ICDSC: Intensive Care Delirium Screening Checklist; Psych: psychiatrist interview; NR: not reported.") |>
  # tab_footnote(md("Mean <u>Med</u> (SD) [Range] {IQR}."), locations = cells_column_labels(columns = c(age_table, pre_mmse))) |>
...
dncr_dat <- dichot_dat |>
  ...
  mutate(
    bar = case_when(
      str_detect(arm, "Dex|Ket|Mel|Ram") ~ bar_prop(neurocog_prop * 100, "#969696"),
      str_detect(arm, "Plac") ~ bar_prop(neurocog_prop * 100, color_3),
  ...
  select(refid, refid_c, year, arm_id, study, study_l, design_f_lab, pre_mmse, country, surg_f, postop_only, arm, pre_mmse, dncr_time, neurocog_total_n, arm_n, arm, neurocog_nper, neurocog_prop, bar, scale_mmse:scale_other_tf)
...
neurocog_allarms_meta_dat <- dncr_dat |> 
  select(refid, study, design_f_lab, year, arm_id, arm, neurocog_total_n, arm_n) |> 
...
dncr_dat <- dncr_dat |> 
  ...
  select(refid, year, design_f_lab, study, study_l, arm_id, arm_n, arm, pre_mmse, scale_mmse:scale_other_tf, dncr_time, neurocog_nper, bar, rr_ci)
...
dncr_dat |> 
  ...
  tab_style(style = list(cell_text(color = color_1)), locations = cells_body(columns = c(arm_n:pre_mmse, neurocog_nper), rows = !str_detect(arm, "Dex|Ket|Mel|Ram|Plac"))) |> 
  tab_style(style = list(cell_text(color = color_3)), locations = cells_body(columns = c(arm_n:pre_mmse, neurocog_nper), rows = str_detect(arm, "Plac"))) |> 
...
dncr_dat |> 
  ...
  mutate(arm = fct_recode(arm, "Plac" = "Plac/None")) |> 
  mutate(study_l = ifelse(row_number() > 1, NA, study_l), 
         across(scale_mmse:scale_other_tf, ~ ifelse(row_number() > 1, NA, .x)),
         dncr_time = ifelse(row_number() > 1, NA, dncr_time), 
                .by = study) |> 
...
<font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention (randomized clinical trials, delayed neurocognitive recovery). </font>

```{r dncr_dex_plac_meta, fig.width = 14, fig.height = 5.5, fig.align = "left", warning = FALSE}
#| eval: true
...
0.5 added to cells of studies without events in one arm.<br/>
...
Note: Gao 2021 identified in the search was presumably an overlapping sample of Gao 2020 and excluded.<br/>
The result might be considered credible and convincing to a skeptic given plausible evidence for risk ratios less than 0.74 (assuming a 25% incidence of delayed neurocognitive recovery; see [Matthews 2019](https://www.tandfonline.com/doi/full/10.1080/00031305.2018.1543136)).
</foot_mg><br/>

::: {.callout-note collapse="true" appearance="minimal" icon="false"}
#### <font size = 4> `r figure_ref()` Dexmedetomidine compared with placebo or no intervention according to risk of bias appraisal (randomized clinical trials; delayed neurocognitive recovery). </font>
...
legend(0.03, 0.02, c("0.1 > p > 0.05", "0.05 > p > 0.01", "< 0.01"), fill = c("#AAB7B8", "#D5DBDB", "#F4F6F6"), bty = "n")
...
<font size = 4> `r table_ref()` Report complications in dexmedetomidine studies — cardiac, MI, stroke, pulmonary, pneumonia, acute kidney injury. </font>
...
adverse_events_dat <- adverse_events_dat |> 
  ...
  mutate(
    ...
    bar = case_when(
      str_detect(arm, "Dex|Ket|Mel|Ram") ~ bar_prop(calc_per, "#969696"),
      str_detect(arm, "Plac|None") ~ bar_prop(calc_per, color_4),
      .default = bar_prop(calc_per, color_2))) |> 
...
adverse_events_dat |> 
  filter(refid %in% mel_ram_refid) |> 
  filter(complication %in% c("cardiac", "pulm", "pneum")) |> 
