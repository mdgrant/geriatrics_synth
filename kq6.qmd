...
```{r nmmForest, fig.width = 6, fig.height = 3.5, fig.align = "center"}
#| eval: true
par(mar = c(1, 1, 1, 1))
...
as_tibble(nmm_league$random) |>
  mutate(
    across(V1:V9, ~ str_replace(.x, " \\[", "<br>\\(")),
    across(V1:V9, ~ str_replace(.x, "\\]", "\\)")),
    across(V1:V9, ~ ifelse(.x == ".", "", .x))
  ) |>
  gt("one") |>
  fmt_markdown(columns = everything()) |>
  tab_style(style = cell_text(align = "center", size = "px(10)"), locations = cells_body(V1:V9)) |>
  ...
  tab_style(style = list(cell_text(color = "maroon", weight = "bold")), locations = cells_body(columns = V9, rows = V9 %in% trts)) |>
  rm_stubhead() |>
  tab_options(column_labels.hidden = TRUE) |>
  gt_theme_mg() |> 
  tab_style(style = cell_text(size = "px(10)"), locations = cells_body(V1:V9))
