...
forest(dex_plac_meta,
  ...
  text.addline2 = paste0("Delirium a designated primary outcome in ", prim_sec_out_fun(prim_delirium, dex_plac_refid), "; a secondary outcome in ", prim_sec_out_fun(prim_delirium, dex_plac_refid), ".")
)
```
...
<font size = 4> `r figure_ref()` Mortalty Dexmedetomidine compared with placebo or no intervention (randomized clinical trials). </font>

```{r dex_mort_meta, fig.dim = c(14, 3.8), fig.align = "center"}
# ```{r nma_dat, out.width = 50%, out.height = 100%}
...
rob_summary(rob2_meta_dat, tool = "ROB2", colour = "colourblind")
```
