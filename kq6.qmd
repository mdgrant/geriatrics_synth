...
<font size = 4> `r table_ref()` Reported complications in dexmedetomidine studies --- cardiac, MI, stroke, pulmonary, pneumonia, and acute kidney injury (randomized clinical trials unless noted). </font>
...
adverse_events_dat |>
  filter(!refid %in% mel_ram_refid) |>
  mutate(
    # complication = factor(complication, levels = c("cardiac", "myoinfarct", "cardarr", "kidneyinj", "stroke", "pulm", "pneum", "pulcongest", "pe", "respfail", "intub"), labels = c("Bradycardia", "MI", "Cardiac Arrest", "AKI", "Stroke", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "PE", "Respiratory Failure", "Reintubation")),
  ...
  gt(id = "one") |>
  row_group_order(groups = c("Bradycardia", "Other arrhythmia", "MI", "Cardiac Arrest", "Stroke", "AKI", "Pulmonary", "Pneumonia", "Pulmonary Congestion", "PE", "Respiratory Failure", "Reintubation")) |>
...
<font size = 4> `r figure_ref()` Risk ratio for other complications --- cardiac, stroke, pulmonary, and acute kidney injury (dexmedetomidine versus placebo or no interventions; randomized clinical trials except and Li 2020). </font>

```{r complication_meta,  fig.width = 14, fig.height = 14.5, fig.align = "left"}
adverse_meta <- metabin(
  n_Dex, arm_n_Dex, n_Plac, arm_n_Plac,
