...
2023-04-09 added studies from search update 2023-03-17; sensitivity analysis by rob for delirium. 
...
# VARIABLE: delirium_inc_prop <lgl> [dichot_dat] if used max of daily
...
`r paste0("Delirium a designated primary outcome in ", prim_sec_out_fun(prim_delirium, dex_plac_refid), "; a secondary outcome in ", prim_sec_out_fun(sec_delirium, dex_plac_refid), ".")`</foot_mg> <a id="figure-4"></a> <br/>
...
```{r nma_dat_rob, fig.width = 14, fig.height = 11.5, fig.align = "left"}
...
```{r mortality_dex_hosp_meta}
#| eval: true
mortality_dex_hosp_meta_dat <- mortality_dat |>
...
trts_mort_dex_hosp <- levels(fct_drop(mortality_dex_hosp_meta_dat$arm))

pairwise_mort_dex_hosp_dat <- pairwise(
  treat = arm,
  event = mort_n,
  n = arm_n,
  studlab = study,
  allstudies = TRUE,
  data = mortality_dex_hosp_meta_dat,
  sm = "RR",
  keep.all.comparisons = TRUE
)
...
mortality_mel_ram_hosp_meta_dat <- mortality_dat |>
  filter(refid %in% mel_ram_mort_refid, mort_period == "Hospital") |>
  rename(arm = drug_recode_abbr) |> 
  filter(!str_detect(arm, "Ulin")) |> 
  select(refid, study, arm, mort_n, arm_n)

trts_mort_mel_ram_hosp <- levels(fct_drop(mortality_mel_ram_hosp_meta_dat$arm))

pairwise_mort_mel_ram_hosp_dat <- pairwise(
  treat = arm,
  event = mort_n,
  n = arm_n,
  studlab = study,
  allstudies = TRUE,
  data = mortality_mel_ram_hosp_meta_dat,
  sm = "RR",
  keep.all.comparisons = TRUE
)
...
::: {.callout-note collapse="true" appearance="minimal" icon="false"}
### <font size = 4> `r figure_ref()` Summary risk of bias from dexmedetomidine randomized clinical trials reporting mortality over all lengths of follow-up. </font>
...
# ```{r nma_dat, out.width = 50%, out.height = 100%}
dex_mort_meta_dat <- pairwise_mort_dex_hosp_dat |> 
  mutate(year = str_extract(study, "\\d{4}")) |> 
  left_join(rob2_meta_dat, by = "refid") |> 
...
<font size = 4> `r figure_ref()` Risk difference for hospital mortality -- melatonin or ramelteon compared with placebo (randomized clinical trials). </font>

```{r mel_ram_mort_meta, fig.dim = c(14, 3.8), fig.align = "center"}
# ```{r nma_dat, out.width = 50%, out.height = 100%}
mel_ram_mort_meta_dat <- pairwise_mort_mel_ram_hosp_dat |> 
  mutate(year = str_extract(study, "\\d{4}")) |> 
  left_join(rob2_meta_dat, by = "refid") |> 
  filter(treat1 == "Plac" & treat2 %in%  c("Mel", "Ram")) |> 
  select(study, year, event2, n2, event1, n1, D1:Overall)
