---
title: "Study/Patient Characteristics Regional versus General Anesthesia"
...
study_char_tab <- study_char_dat |> 
  filter(refid %in% no_linked_refids) |>
  mutate(
...
anesthetic <- study_arm_dat |> 
  ...
  mutate(
    ...
    volatile = kq3_gen_type %in% c("inhalation", "tiva_inhal"),
    tiva = kq3_gen_type %in% c("tiva", "tiva_inhal")
  ) |> 
...
table <- study_char_tab |> 
  ...
  summarize(
    ...
    "Country" = " ",
    "Non-very-high HDI Country" = n_per_tf(low_r),
    # expression(paste("Non-very-high HDI")) = n_per_tf(low_r),
    "Funding" = " ",
...
table |> 
 ...
 gt_theme_mg() |> 
 tab_footnote("Studies with multiple publications counted once.") |>
 tab_footnote(md("Non very-high [Human Development Index](https://en.wikipedia.org/wiki/List_of_countries_by_Human_Development_Index) country."), locations = cells_body(columns = c(characteristic), rows = characteristic == "Non-very-high HDI Country"))

# writexl::write_xlsx(table, "/Users/mgrant/Desktop/study_char_tab_kq3.xlsx")
```

### Patient Charcteristics

<font size = 4> `r table_ref()` Patient characteristics. </font>
...
surg <- surgs |> 
  ...
  mutate(
    ...
    characteristic = ifelse(characteristic == "Various", "Various (>4)", characteristic),
